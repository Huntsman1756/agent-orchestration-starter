...
export function buildReviewEnvelope(input: ReviewEnvelopeInputV4): ReviewEnvelopeV4 {
  ...
  if (keys.length !== expected.length || keys.some((key, index) => key !== expected[index]))
    invalid('review envelope input contains hidden context');
  if (
    !hash(input.capability_snapshot_hash) ||
    !hash(input.diff_hash) ||
    !hash(input.tree_hash) ||
    Buffer.byteLength(input.complete_diff, 'utf8') > 2 * 1024 * 1024 ||
    input.changed_files.length > 256 ||
    new Set(input.changed_files.map((path) => path.toLowerCase())).size !== input.changed_files.length ||
    input.changed_files.some((path) => !isNormalizedRepositoryRelativePathV4(path)) ||
    input.validation_results.length < 1 ||
    input.validation_results.length > 64 ||
    new Set(input.validation_results.map((result) => result.validation_id)).size !== input.validation_results.length ||
    input.validation_results.some(
      (result) => !result.passed || !hash(result.result_hash) || result.validated_tree_hash !== input.tree_hash,
    ) ||
    input.unresolved_findings.length > 128 ||
    new Set(input.unresolved_findings.map((finding) => finding.id)).size !== input.unresolved_findings.length
  )
    invalid('review evidence is malformed or mismatched');
  let contract: RuntimeWorkContractV4;
  try {
    contract = deepFreeze(loadRuntimeWorkContractV4(structuredClone(input.contract)));
  } catch {
    return invalid('work contract is not strict V4 evidence');
  }
  ...
  const body = {
    ...
    complete_diff: input.complete_diff,
