...
export interface AttestationVerificationInputV4 {
  readonly attestation: unknown;
  readonly current: AttestationCurrentEvidenceV4;
}
...
export function verifyReviewAttestation(input: AttestationVerificationInputV4): ReviewAttestationV4 {
  ...
  try {
    attestation = loadReviewAttestationV4(structuredClone(input.attestation));
  } catch {
    return invalid('attestation does not match the strict contract');
  }
  ...
  if (
    ...
    attestation.contract_hash !== input.current.contract_hash ||
    attestation.base_sha !== input.current.base_sha ||
    attestation.reviewed_tree_hash !== input.current.tree_hash ||
    attestation.reviewed_diff_hash !== input.current.diff_hash ||
    ...
    (attestation.decision === 'ACCEPT' &&
      (attestation.unresolved_finding_ids.length > 0 || attestation.requested_context_hashes.length > 0))
  )
    invalid('attestation is stale, forged, reused, or mismatched');
  return Object.freeze({
    ...attestation,
    findings: Object.freeze([...attestation.findings]),
    requested_context_hashes: Object.freeze([...attestation.requested_context_hashes]),
    unresolved_finding_ids: Object.freeze([...attestation.unresolved_finding_ids]),
  });
}
