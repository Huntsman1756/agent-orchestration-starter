...
export interface DaemonOwnedPipelineV4 {
  advance(runId: string): Promise<void>;
  abort(runId: string): Promise<void>;
}
export interface OrchestratorDaemonPortV4 {
  submit(command: Extract<BrokerCommandV4, { type: 'RUN_CODING_TASK' }>): Promise<BrokerReplyV4>;
...
function runId(value: string): void {
  if (!/^run_[A-Za-z0-9_-]{16,96}$/.test(value)) throw new Error('INVALID_CONTRACT: run_id is invalid');
}
...
export function createRuntimeOrchestratorV4(deps: RuntimeOrchestratorDependenciesV4): RuntimeOrchestratorV4 {
  ...
  return Object.freeze({
    start: async (supplied: RuntimeTaskRequestV4) => {
      ...
      const command: Extract<BrokerCommandV4, { type: 'RUN_CODING_TASK' }> = {
        type: 'RUN_CODING_TASK',
        command_id: (deps.command_id ?? (() => `mcp-${randomBytes(16).toString('hex')}`))(),
        request,
      };
    ...
    resume: async (id: string) => {
      runId(id);
      const current = await deps.daemon.status(id);
      if (!terminal(current.state)) schedule(id);
      return current;
    },
    abort: async (id: string) => {
      runId(id);
      await deps.pipeline.abort(id);
      return await deps.daemon.status(id);
    },
  });
}
