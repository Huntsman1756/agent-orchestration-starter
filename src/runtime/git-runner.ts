...
function allowedGitArgv(argv: readonly string[]): boolean {
  ...
  if (command === 'worktree') {
    return (
      (rest.length === 5 && rest[0] === 'add' && rest[1] === '-b' && branchPattern.test(rest[2] ?? '') && shaPattern.test(rest[4] ?? '')) ||
  ...
  if (command === 'branch') {
    return (
      (rest.length === 2 && branchPattern.test(rest[0] ?? '') && shaPattern.test(rest[1] ?? '')) ||
      (rest.length === 2 && rest[0] === '-D' && branchPattern.test(rest[1] ?? ''))
    );
  }
  if (command === 'show-ref')
    return rest.length === 2 && rest[0] === '--verify' && /^refs\/heads\/codex\/auto\/[A-Za-z0-9_-]{1,96}$/.test(rest[1] ?? '');
