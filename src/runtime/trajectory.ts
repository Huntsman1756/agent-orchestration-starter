...
export interface RuntimeGraphNodeV4 {
  readonly node_id: string;
  readonly story_hash: string;
  readonly status: 'PENDING' | 'RETRY' | 'ACCEPTED' | 'ESCALATED';
  readonly attempts: number;
}
export interface RuntimeGraphEdgeV4 {
  readonly from: string;
  readonly to: string;
  readonly kind: 'DEPENDS_ON';
}
export interface RuntimeExecutionGraphV4 {
  readonly schema_version: 4;
  readonly run_id: string;
  readonly plan_hash: string;
  readonly status: 'COMPLETE' | 'ESCALATE' | 'IN_PROGRESS';
  readonly nodes: readonly RuntimeGraphNodeV4[];
  readonly edges: readonly RuntimeGraphEdgeV4[];
  readonly frontier_decision_hashes: readonly string[];
...
export interface RuntimeTraceExportV4 {
  readonly schema_version: 4;
  readonly run_id: string;
  readonly contract_hash: string;
  readonly spans: readonly RuntimeTraceSpanV4[];
  readonly trace_hash: string;
}
export interface TrajectoryRuleResultV4 {
  readonly rule_id: 'TELEMETRY_CHAIN' | 'TERMINAL_LAST' | 'EXECUTION_BALANCED' | 'VALIDATION_BEFORE_REVIEW' | 'ITERATION_GRAPH';
  readonly outcome: 'PASS' | 'FAIL';
  readonly evidence_hashes: readonly string[];
}
export interface RuntimeTrajectoryEvaluationV4 {
  readonly schema_version: 4;
  readonly run_id: string;
  readonly outcome: 'PASS' | 'FAIL';
  readonly rules: readonly TrajectoryRuleResultV4[];
  readonly graph_hash: string | null;
  readonly trace_hash: string | null;
...
export function buildRuntimeExecutionGraphV4(input: {
  ...
  const nodes = plan.stories.map((story): RuntimeGraphNodeV4 =>
    Object.freeze({
      node_id: story.story_id,
      story_hash: story.story_hash,
      status: accepted.has(story.story_id)
        ? 'ACCEPTED'
        : lastOutcome.get(story.story_id) === 'ESCALATE'
          ? 'ESCALATED'
          : lastOutcome.get(story.story_id) === 'RETRY'
            ? 'RETRY'
            : 'PENDING',
      attempts: snapshot.attempts_by_story[story.story_id] ?? 0,
    }),
  );
  const edges = plan.stories.flatMap((story) =>
    story.depends_on.map((dependency): RuntimeGraphEdgeV4 => Object.freeze({ from: dependency, to: story.story_id, kind: 'DEPENDS_ON' })),
  );
  ...
  const body = {
    ...
    nodes,
    edges,
    frontier_decision_hashes: snapshot.frontier_decisions.map((decision) => decision.decision_hash),
  };
...
export function exportRuntimeTraceV4(supplied: readonly RuntimeEventV4[]): RuntimeTraceExportV4 {
  ...
  const spans = verified.map((event, index): RuntimeTraceSpanV4 => {
    ...
    const attributes: Record<string, string | number> = {
      ...
      evidence_count: event.evidence_hashes.length,
      ...event.counters,
    };
    ...
    return freeze({
      trace_id: traceId,
      span_id: event.event_hash.slice(0, 16),
      parent_span_id: index === 0 ? null : verified[index - 1]!.event_hash.slice(0, 16),
      name: event.type,
      recorded_at: event.recorded_at,
      duration_ms: event.duration_ms ?? null,
      status: event.type === 'RUN_FAILED' || event.type === 'RUN_ABORTED' ? 'ERROR' : 'OK',
      attributes,
...
export function evaluateRuntimeTrajectoryV4(input: {
  contract: RuntimeWorkContractV4;
  worker: WorkerCapabilityV4;
  plan: IterativeStoryPlanV4;
  initial_tree_hash: string;
  story_events: readonly StoryIterationEventV4[];
  runtime_events: readonly RuntimeEventV4[];
  review_control_mode?: 'AUTONOMOUS_BROKER' | 'FRONTIER_LED';
  frontier_decisions?: readonly FrontierDecisionEventV4[];
}): RuntimeTrajectoryEvaluationV4 {
  const rules: TrajectoryRuleResultV4[] = [];
  let trace: RuntimeTraceExportV4 | null = null;
  try {
    trace = exportRuntimeTraceV4(input.runtime_events);
    rules.push({ rule_id: 'TELEMETRY_CHAIN', outcome: 'PASS', evidence_hashes: [trace.trace_hash] });
  } catch {
    rules.push({ rule_id: 'TELEMETRY_CHAIN', outcome: 'FAIL', evidence_hashes: [] });
  }
  ...
  rules.push({
    rule_id: 'TERMINAL_LAST',
    outcome: terminalPass ? 'PASS' : 'FAIL',
    evidence_hashes: terminalPass && trace !== null ? [trace.trace_hash] : [],
  });
  let activeExecutions = 0;
  let balanced = true;
  for (const type of types) {
    if (type === 'EXECUTION_STARTED') activeExecutions += 1;
    if (type === 'EXECUTION_COMPLETED') activeExecutions -= 1;
    if (activeExecutions < 0 || activeExecutions > 1) balanced = false;
  }
  if (activeExecutions !== 0) balanced = false;
  rules.push({
    rule_id: 'EXECUTION_BALANCED',
    outcome: balanced ? 'PASS' : 'FAIL',
    evidence_hashes: balanced && trace !== null ? [trace.trace_hash] : [],
  });
  ...
  for (const [index, type] of types.entries()) {
    ...
    if (type === 'REVIEW_COMPLETED') {
      if (!reviewActive || lastValidation <= lastReview) reviewOrder = false;
      reviewActive = false;
      lastReview = index;
    }
  }
  if (reviewActive) reviewOrder = false;
  rules.push({
    rule_id: 'VALIDATION_BEFORE_REVIEW',
    outcome: reviewOrder ? 'PASS' : 'FAIL',
    evidence_hashes: reviewOrder && trace !== null ? [trace.trace_hash] : [],
  });
  let graph: RuntimeExecutionGraphV4 | null = null;
  try {
    graph = buildRuntimeExecutionGraphV4({
      contract: input.contract,
      worker: input.worker,
      plan: input.plan,
      initial_tree_hash: input.initial_tree_hash,
      events: input.story_events,
      review_control_mode: input.review_control_mode,
      frontier_decisions: input.frontier_decisions,
    });
    rules.push({ rule_id: 'ITERATION_GRAPH', outcome: 'PASS', evidence_hashes: [graph.graph_hash] });
  } catch {
    rules.push({ rule_id: 'ITERATION_GRAPH', outcome: 'FAIL', evidence_hashes: [] });
  }
  const body = {
    schema_version: 4 as const,
    run_id: input.contract.run_id,
    outcome: rules.every((rule) => rule.outcome === 'PASS') ? ('PASS' as const) : ('FAIL' as const),
    rules,
    graph_hash: graph?.graph_hash ?? null,
    trace_hash: trace?.trace_hash ?? null,
  };
