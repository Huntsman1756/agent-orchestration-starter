...
const uniqueHashes = z
  .array(hash)
  .max(64)
  .refine((values) => new Set(values).size === values.length, 'hashes must be unique');
...
const bodySchema = z
  ...
  .superRefine((value, context) => {
    if (value.disposition === 'DELEGATED') {
      if (value.route === 'FRONTIER_EXECUTION')
        context.addIssue({ code: 'custom', message: 'delegated evidence cannot claim the frontier-only route' });
      if (value.worker_capability_hash === null)
        context.addIssue({ code: 'custom', message: 'delegated evidence requires worker capability identity' });
      if (value.accepted_story_receipt_hashes.length < 1)
        context.addIssue({ code: 'custom', message: 'delegated evidence requires accepted worker receipts' });
      if (value.exemption !== null) context.addIssue({ code: 'custom', message: 'delegated evidence cannot carry an exemption' });
    } else {
      if (value.route !== 'FRONTIER_EXECUTION')
        context.addIssue({ code: 'custom', message: 'only frontier execution may use an exemption' });
      if (value.exemption === null)
        context.addIssue({ code: 'custom', message: 'frontier-only execution requires explicit authority evidence' });
      if (value.accepted_story_receipt_hashes.length !== 0)
        context.addIssue({ code: 'custom', message: 'frontier-only evidence cannot claim delegated receipts' });
    }
  });

const evidenceSchema = bodySchema.and(
  z
    .object({
...
export function createDelegationProvenanceV4(
  input: Omit<DelegationProvenanceBodyV4, 'schema_version' | 'kind'>,
  signer: DelegationProvenanceSignerV4,
): DelegationProvenanceV4 {
...
export function verifyDelegationProvenanceV4(
  input: unknown,
  binding: DelegationProvenanceBindingV4,
  ...
  let key;
  try {
    key = createPublicKey(typeof publicKey === 'string' ? publicKey : Buffer.from(publicKey));
  } catch {
    invalid('trusted public key is invalid');
  }
  ...
  if (body.commit_sha !== binding.commit_sha || body.git_tree_sha !== binding.git_tree_sha)
    invalid('commit or tree binding does not match');
  if (body.policy_hash !== binding.policy_hash || body.profile_hash !== binding.profile_hash)
    invalid('policy or profile binding does not match');
