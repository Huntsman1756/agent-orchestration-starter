...
import {
  validateRuntimeHostComponentPortV4,
  type RuntimeHostComponentPortV4,
  type RuntimeHostComponentSetV4,
} from './host-component-ports.js';
...
export async function loadRuntimeHostComponentsV4(input: {
  ...
  for (const binding of composition.components) {
    ...
    const module = (await import(`${pathToFileURL(binding.path).href}?certification=${binding.certificationHash}`)) as Record<
      string,
      unknown
    >;
    if (Object.keys(module).join(',') !== 'createRuntimeHostComponentV4' || typeof module.createRuntimeHostComponentV4 !== 'function')
      unavailable(`host component ${binding.id} does not export exactly createRuntimeHostComponentV4`);
  ...
  if (RUNTIME_HOST_COMPONENT_IDS_V4.some((id) => loadedComponents[id] === undefined))
    unavailable('the host component composition is incomplete');
