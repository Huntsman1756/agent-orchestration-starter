...
export function createValidationRunner(deps: ValidationRunnerDependenciesV4): ValidationRunnerV4 {
  if (!Number.isSafeInteger(deps.preview_bytes) || deps.preview_bytes < 0 || deps.preview_bytes > 64 * 1024)
    failed('preview byte policy is invalid');
  return Object.freeze({
    run: async (input: ValidationRunInputV4): Promise<ValidationResultV4> => {
      ...
      if (!/^[a-f0-9]{64}$/.test(input.tree_hash) || !/^[a-f0-9]{64}$/.test(input.expected_sandbox_policy_hash))
        failed('validation identity is invalid');
      ...
      const run =
        beforeTreeHash === input.tree_hash
          ? await deps.sandbox.run({
              execution_id: input.execution_id,
              profile: 'VALIDATION_UNTRUSTED',
              argv: input.validation.argv,
              working_directory:
                input.validation.working_directory === '.' ? '/capsule/repo' : `/capsule/repo/${input.validation.working_directory}`,
          ...
          : {
              ...
              timed_out: false,
              stdout: '',
              stderr: 'tree changed before validation',
      ...
      const artifactsValid = await Promise.all([deps.artifacts.verify(stdoutArtifact), deps.artifacts.verify(stderrArtifact)]).then(
