...
export interface FrontierExecutorDependenciesV4 {
  ...
  readonly validate: (
    ...
    capsule: ExecutorCapsuleV4,
    attempt: ExecutorAttemptResultV4,
  ) => Promise<readonly FrontierValidationEvidenceV4[]>;
  readonly fresh_review: (
    contract: RuntimeWorkContractV4,
    capsule: ExecutorCapsuleV4,
    attempt: ExecutorAttemptResultV4,
    validation: readonly FrontierValidationEvidenceV4[],
  ) => Promise<FrontierReviewEvidenceV4>;
  readonly story_id?: string;
  readonly on_repair_packet?: (input: {
    readonly packet: RepairPacketV4;
    readonly attempt: ExecutorAttemptResultV4;
    readonly validation: readonly FrontierValidationEvidenceV4[];
  }) => Promise<void> | void;
...
export function createFrontierExecutor(deps: FrontierExecutorDependenciesV4): FrontierExecutorV4 {
  const state = (value: FrontierStateV4): void => {
    deps.on_state?.(value);
  };
  ...
  return Object.freeze({
    execute: async (contract: RuntimeWorkContractV4, capsule: ExecutorCapsuleV4): Promise<ExecutorAttemptResultV4> => {
      ...
      if (
        validation.length === 0 ||
        validation.some((result) => result.validated_tree_hash !== attempt.diff.tree_hash || !/^[a-f0-9]{64}$/.test(result.result_hash))
      ) {
      ...
      if (
        ...
        review.reviewer_session_id === attempt.session_id ||
        review.reviewed_tree_hash !== attempt.diff.tree_hash ||
        review.reviewed_diff_hash !== attempt.diff.diff_hash
      ) {
