...
import { buildReviewEnvelope, type ReviewEnvelopeV4 } from './review-envelope.js';
...
export function buildBrokerReviewPacket(input: BrokerReviewPacketInputV4): BrokerReviewPacketV4 {
  ...
  if (input.result.state !== 'REVIEW_ACCEPTED')
  ...
  if (envelope.unresolved_findings.length > 0)
    throw new Error('REVIEW_PACKET_UNAVAILABLE: unresolved review findings must be repaired before verdict submission');
  const contract = loadRuntimeWorkContractV4(structuredClone(envelope.contract));
  if (contract.run_id !== input.result.run_id || contract.request_id !== input.result.request_id)
    invalid('packet identity is not bound to the run');
  if (contract.contract_hash !== input.result.contract_hash || envelope.contract.contract_hash !== input.result.contract_hash)
    invalid('packet contract hash does not match the durable result');
  if (
    envelope.base_sha !== input.result.base_sha ||
    envelope.diff_hash !== input.result.diff_hash ||
    envelope.tree_hash !== input.result.tree_hash
  )
    invalid('packet diff/tree evidence does not match the durable result');
  if (
    envelope.changed_files.length !== input.result.changed_files.length ||
    envelope.changed_files.some((path, index) => path !== input.result.changed_files[index])
  )
    invalid('packet changed files do not match the durable result');
  if (envelope.validation_manifest.length !== input.result.validation_results.length)
    invalid('packet validation manifest length does not match the durable result');
  for (const [index, validation] of envelope.validation_manifest.entries()) {
    const result = input.result.validation_results[index];
    if (
      result === undefined ||
      result.exit_code !== 0 ||
      validation.validation_id !== result.validation_id ||
      !validation.passed ||
      validation.result_hash !== result.result_hash ||
      validation.validated_tree_hash !== input.result.tree_hash
    ) {
...
export function loadBrokerReviewPacketV4(value: unknown): BrokerReviewPacketV4 {
  ...
  const normalized = buildBrokerReviewPacket({
    result: {
      ...
      changed_files: envelope.changed_files,
      validation_results: envelope.validation_manifest.map((item) => ({
        validation_id: item.validation_id,
        exit_code: item.passed ? 0 : 1,
  ...
  if (
    normalized.packet_hash !== hash(packet.packet_hash, 'packet_hash') ||
    normalized.request_id !== requestId(packet.request_id) ||
    normalized.run_id !== runId(packet.run_id) ||
    normalized.capability_snapshot_hash !== hash(packet.capability_snapshot_hash, 'capability_snapshot_hash') ||
    normalized.validation_manifest_hash !== hash(packet.validation_manifest_hash, 'validation_manifest_hash') ||
    normalized.base_sha !== packet.base_sha
  ) {
...
export function createBrokerVerdictRecord(input: BrokerVerdictInputV4, packet: BrokerReviewPacketV4): BrokerVerdictRecordV4 {
  ...
  if (typeof input.reason !== 'string' || input.reason.trim().length < 1 || input.reason.length > 4_000 || input.reason.includes('\u0000'))
