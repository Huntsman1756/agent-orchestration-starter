...
const modelGuidanceSchema = z
  .object({
    id: identifierSchema,
    revision: identifierSchema,
    sourceUrls: uniqueArray(
      z
        .url()
        .max(2_048)
        .refine((value) => value.startsWith('https://'), 'guidance sources must use HTTPS'),
      { min: 1, max: 8 },
    ),
...
export const normalizedRepositoryRelativePathV4Schema = z
  .string()
  .min(1)
  .refine(isNormalizedRepositoryRelativePathV4, 'path must be a normalized repository-relative exact path');
...
const implementationTargetSchema = z
  .object({
...
function refineSddStrictPolicy(value: SddStrictPolicyValue, context: z.RefinementCtx): void {
  ...
  if (targets.size !== value.implementation_targets.length) {
    context.addIssue({
      ...
      message: 'implementation target paths must be unique case-insensitively',
    });
  }
  for (const path of acceptanceTests) {
    if (targets.has(path))
      context.addIssue({ code: 'custom', path: ['implementation_targets'], message: 'acceptance tests cannot be implementation targets' });
  }
  ...
  if (allowed.length !== implementation.length || allowed.some((entry, index) => entry !== implementation[index])) {
    context.addIssue({
      code: 'custom',
      path: ['implementation_targets'],
      message: 'implementation_targets must exactly mirror allowed_changes',
    });
  }
}
...
export const runtimeProfileV4Schema = z
  .object({
  ...
  .superRefine((value, context) => {
    ...
    for (const role of roles) {
      ...
      if (binding.permissions !== expected[role].permissions) {
        context.addIssue({
          code: 'custom',
          path: ['bindings', role, 'permissions'],
          message: `${role} must use ${expected[role].permissions} permissions`,
        });
      }
      if (binding.permissions === 'contract-write' && binding.execution === undefined) {
        context.addIssue({
          code: 'custom',
          path: ['bindings', role, 'execution'],
          message: `${role} must declare an explicit qualified execution envelope`,
        });
      }
      ...
      if (binding.harness !== 'codex' || binding.permissions !== 'read-only' || (role !== 'orchestrator' && role !== 'reviewer')) {
        context.addIssue({
          code: 'custom',
          path: ['bindings', role, 'authentication'],
          message: 'ChatGPT subscription authentication is restricted to read-only Codex orchestrator and reviewer bindings',
        });
      }
    }
  });
...
export const runtimeRepositoryPolicyV4Schema = z
  .object({
    schemaVersion: z.literal(4),
    ...
    routing: z
      ...
      .strict(),
    validation: z
      .record(identifierSchema, validationSchema)
      .refine((value) => Object.keys(value).length > 0, 'validation must not be empty'),
    ...
    sandbox: z
      ...
      .strict(),
    instructions: z
      .object({ approvedSources: uniqueArray(normalizedRepositoryRelativePathV4Schema.max(512), { min: 1, max: 64 }) })
      .strict(),
    publication: z
      .object({
        enabled: z.boolean(),
...
export const runtimeWorkContractV4Schema = z
  .object({
    ...taskRequestFields,
    run_id: runIdSchema,
    ...
    sandbox_profile_hashes: z
      .record(identifierSchema, hashSchema)
      .refine((value) => Object.keys(value).length > 0, 'sandbox profile hashes must not be empty'),
    policy_hash: hashSchema,
    profile_hash: hashSchema,
    contract_hash: hashSchema,
  })
  .strict()
  .superRefine(refineSddStrictPolicy);
...
export const runtimeResultV4Schema = z
  .object({
    run_id: runIdSchema,
    request_id: requestIdSchema,
    ...
    profile_hash: hashSchema,
    attempts: z
      .array(z.object({ attempt: z.number().int().positive(), executor_binding_ref: identifierSchema, result_hash: hashSchema }).strict())
      .max(3),
    validation_results: z
      .array(z.object({ validation_id: identifierSchema, exit_code: z.number().int(), result_hash: hashSchema }).strict())
      .max(64),
...
export const reviewAttestationV4Schema = z
  .object({
    ...
    findings: z
      .array(z.object({ id: identifierSchema, severity: identifierSchema, message: z.string().min(1).max(2_000) }).strict())
      .max(128),
