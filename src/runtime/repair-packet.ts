...
const failureSignatureFindingSchema = z
  .object({
    source: z.enum(['VALIDATION', 'REVIEW']),
    category_code: identifier,
    path: normalizedRepositoryRelativePathV4Schema.max(512).nullable(),
  ...
  .object({
    ...
    findings: z
      ...
      .superRefine((values, context) => {
        if (new Set(values.map((value) => value.finding_id)).size !== values.length)
          context.addIssue({ code: 'custom', message: 'finding IDs must be unique' });
        if (new Set(values.map((value) => value.evidence_hash)).size !== values.length)
          context.addIssue({ code: 'custom', message: 'finding evidence must be unique' });
      }),
  })
  .strict();
...
export const SHIFT_LEFT_REPAIR_INSTRUCTION_V4 =
  'Las pruebas pasan, pero el código viola las políticas estáticas de calidad/seguridad. Repara los siguientes errores de lint antes de reclamar completitud.';
...
export function createNormalizedFailureSignatureV4<T extends FailureSignatureFindingV4>(findings: readonly T[]): string {
  ...
  const canonical = [...new Map(parsed.map((finding) => [JSON.stringify(finding), finding])).values()].sort((left, right) => {
