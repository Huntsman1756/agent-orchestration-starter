...
export async function verifyRuntimeRepositoryActivationV4(path: string): Promise<{ activation: RuntimeRepositoryActivationV4; installation: RuntimeHostInstallationManifestV4 }> {
  ...
  if (canonicalPath !== join(repositoryRoot, '.agent-orchestration', 'activation-v4.json')) invalid('activation manifest is outside its canonical repository location');
  for (const recorded of [activation.policyPath, activation.profilePath, activation.worktreeParent]) {
    if (await realpath(recorded).catch(() => invalid('an activated path is unavailable')) !== recorded) invalid('an activated path drifted from its canonical location');
  }
...
async function sourceFiles(root: string): Promise<readonly RuntimeInstalledFileV4[]> {
  const selected = [
    { path: 'dist/host/agent-orchestration.mjs', required: true }, { path: 'contracts', required: true },
    { path: 'dist/native', required: false }, { path: 'LICENSE', required: true }, { path: 'package.json', required: true },
  ];
  const files: RuntimeInstalledFileV4[] = [];
  let total = 0;
  async function visit(absolute: string): Promise<void> {
    const metadata = await lstat(absolute).catch(() => invalid(`required bundle path is unavailable: ${portable(relative(root, absolute))}`));
  ...
  for (const value of selected) {
    ...
    if (!value.required && await lstat(absolute).then(() => false, () => true)) continue;
...
export function loadRuntimeHostInstallationV4(value: unknown): RuntimeHostInstallationManifestV4 {
  ...
  const files = item.files.map((raw) => {
    ...
    if (Object.keys(file).sort().join(',') !== 'path,sha256,size' || typeof file.path !== 'string' || !/^[A-Za-z0-9._-]+(?:\/[A-Za-z0-9._-]+)*$/u.test(file.path) || file.path.split('/').some((part) => part === '.' || part === '..') || !hashPattern.test(String(file.sha256)) || !Number.isSafeInteger(file.size) || Number(file.size) < 0) invalid('installed file is invalid');
...
export async function verifyRuntimeHostInstallationV4(manifestInput: RuntimeHostInstallationManifestV4): Promise<string> {
  ...
  if (!rootMetadata.isDirectory() || rootMetadata.isSymbolicLink() || await realpath(manifest.root) !== manifest.root) invalid('installation root is unsafe');
  ...
  if (manifest.hostDriver !== null && (await fileHash(manifest.hostDriver.path).catch(() => invalid('host driver is unavailable'))).sha256 !== manifest.hostDriver.sha256) invalid('host driver drifted');
  return manifest.installationHash;
}

async function loadYaml(path: string): Promise<unknown> { return parseYaml(await readFile(path, 'utf8')); }
...
export async function activateRuntimeRepositoryV4(input: { repositoryRoot: string; policyPath: string; profilePath: string; worktreeParent: string; installationManifest: string; hostRoot: string; target: RuntimeActivationTargetV4; activatedAt: string }): Promise<RuntimeRepositoryActivationV4> {
  ...
  if (within(repositoryRoot, worktreeParent) || within(worktreeParent, repositoryRoot)) invalid('worktree parent must be outside the repository tree');
  const manifestPath = await realpath(resolve(input.installationManifest)).catch(() => invalid('installation manifest cannot be canonicalized'));
  ...
  if (!['low','medium','high','xhigh','max'].includes(reasoningEffort)) invalid('orchestrator guidance is not compatible with Codex project activation');
  const codex = renderCodexProjectConfig({ frontier_model: profile.bindings.orchestrator.model, reasoning_effort: reasoningEffort as 'low'|'medium'|'high'|'xhigh'|'max', runtime_entrypoint: installation.entrypoint, activation_manifest: activationPath });
  const priorCodex = await readFile(codexPath, 'utf8').catch(() => null);
  if (priorCodex !== null && priorCodex !== codex.content) invalid(`existing ${portable(relative(repositoryRoot, codexPath))} is unmanaged; merge the generated MCP block manually`);
...
export function activationManifestPathV4(repositoryRoot: string): string { return join(resolve(repositoryRoot), '.agent-orchestration', 'activation-v4.json'); }
