    ...
    review: async (input: ReviewInputV4): Promise<ReviewOutcomeV4> => {
      ...
      try {
        for (let round = 0; round < 2; round += 1) {
          ...
          const run = await deps.sandbox.run({ execution_id: round === 0 ? input.execution_id : `${input.execution_id.slice(0, 88)}_context`, profile: 'REVIEW_CAPSULE', argv: [...deps.harness_argv, 'exec', '--ephemeral', '--ignore-user-config', '--ignore-rules', '--sandbox', 'read-only', '--skip-git-repo-check', '--output-schema', '/capsule/review-attestation-v4.schema.json', '--json', '--cd', '/capsule', '--model', input.binding.binding.model, ...codexModelConfigArgvV4(input.binding.binding.guidance), prompt], working_directory: '/capsule', environment: Object.freeze({ ...lease.environment, HOME: '/capsule/home', TMPDIR: '/capsule/tmp', NO_COLOR: '1' }), mounts: [{ source: capsule.root, target: '/capsule', access: 'READ_ONLY' }], network: { mode: 'INTERNAL', name: lease.internal_network }, timeout_ms: 300_000, max_output_bytes: 2 * 1024 * 1024 });
