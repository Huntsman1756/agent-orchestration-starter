...
const historicalOracleSchema = z
  .object({
...
const usageRoleBindingsSchema = z
  .object({
    allowed_binding_refs: z.array(identifier).min(1).max(128),
  })
  .strict();

const providerUsagePolicySchema = z
  .object({
...
const usageEventBindingSchema = z
  .object({
    usage_id: identifier,
    run_id: identifier,
    event_id: identifier,
...
export const dogfoodStopEventV1Schema = z
  .object(dogfoodStopEventShape)
  .strict()
  .superRefine((event, context) => {
    ...
    if (
      event.last_completed_schedule_ordinal === 0 &&
      (event.stop_condition === 'CRITICAL_FALSE_ACCEPTANCE' || event.stop_condition === 'CROSS_RUN_CONTAMINATION')
    ) {
      context.addIssue({
        code: 'custom',
        path: ['stop_condition'],
        message: `${event.stop_condition} requires a completed triggering run`,
      });
    }
    if (event.last_completed_schedule_ordinal > 0 && event.triggering_run_id === null) {
...
function scheduleFor(input: Pick<DogfoodManifestInputV1, 'cases' | 'scheduling'>): DogfoodManifestV1['schedule'] {
  const rankedCases = [...input.cases].sort((left, right) =>
    hashCanonical({
  ...
  for (const [index, currentCase] of rankedCases.entries()) {
    const rank = (currentStrategy: 'orchestrated' | 'frontier_execution') =>
      hashCanonical({
        algorithm_version: input.scheduling.algorithm_version,
        assignment_seed: input.scheduling.assignment_seed,
    ...
    const routeOrder =
      rank('orchestrated').localeCompare(rank('frontier_execution')) < 0
...
function scheduleErrors(manifest: DogfoodManifestV1): string[] {
  ...
  const expected = new Set(
    manifest.cases.flatMap((currentCase) => [`${currentCase.case_id}:orchestrated`, `${currentCase.case_id}:frontier_execution`]),
  );
  ...
  if (actual.size !== manifest.schedule.length || [...expected].some((value) => !actual.has(value)))
    errors.push('schedule must contain each case and strategy exactly once');
  for (const [index, entry] of manifest.schedule.entries()) {
    ...
    if (!manifest.cases.some((currentCase) => currentCase.case_id === entry.case_id))
      errors.push(`schedule references unknown case: ${entry.case_id}`);
  }
  ...
  for (const entry of manifest.schedule) {
    ...
    if (consecutive > manifest.scheduling.max_consecutive_same_strategy)
      errors.push('schedule is not interleaved within the configured bound');
  }
...
function manifestErrors(manifest: DogfoodManifestV1): string[] {
  ...
  if (!exactSet(manifest.required_metrics, DOGFOOD_REQUIRED_METRICS_V1))
    errors.push('required metrics must be the frozen dogfood metric set');
  ...
  if (
    new Set(manifest.route_bindings.map((binding) => binding.strategy)).size !== 2 ||
  ...
  if (topology.binding_registry_hash !== hashCanonical(topologyRegistry))
    errors.push('provider usage policy registry hash does not match canonical registry content');
  ...
  for (const bindingRef of requiredUsageRefs)
    if (!topologyRefs.has(bindingRef)) errors.push(`provider usage policy omits required binding: ${bindingRef}`);
  ...
  if (usageRefs.size !== topologyRefs.size || [...topologyRefs].some((bindingRef) => !usageRefs.has(bindingRef)))
    errors.push('cost policy usage binding references must match the frozen provider usage registry');
  ...
  for (const [role, refs] of [
    ...
    ['reviewer', reviewerRefs],
    ['executor', executorRefs],
  ] as const) {
    for (const bindingRef of refs)
      if (!topologyRefs.has(bindingRef)) errors.push(`${role} usage policy references unknown binding: ${bindingRef}`);
  }
  const allRoleRefs = new Set([...orchestratorRefs, ...reviewerRefs, ...executorRefs]);
  for (const bindingRef of topologyRefs)
    if (!allRoleRefs.has(bindingRef)) errors.push(`provider usage registry contains an unassigned binding: ${bindingRef}`);
  for (const currentStrategy of ['orchestrated', 'frontier_execution'] as const) {
    ...
    if (new Set(requiredRoles).size !== requiredRoles.length)
      errors.push(`${currentStrategy} provider usage required roles must be unique`);
    for (const requiredRole of ['executor', 'reviewer'] as const) {
      if (!requiredRoles.includes(requiredRole))
        errors.push(`${currentStrategy} provider usage policy must require the ${requiredRole} role`);
    }
  }
  if (!topology.required_usage_roles.orchestrated.includes('orchestrator'))
    errors.push('orchestrated provider usage policy must require the orchestrator role');
  ...
  for (const currentStrategy of ['orchestrated', 'frontier_execution'] as const) {
    ...
    if (routeBinding && topology.roles.executor[currentStrategy] !== routeBinding.binding_ref)
      errors.push(`executor usage policy does not match the ${currentStrategy} route binding`);
    const registryBinding = routeBinding ? topologyRegistry.find((binding) => binding.binding_ref === routeBinding.binding_ref) : undefined;
    if (routeBinding && registryBinding && registryBinding.profile_hash !== routeBinding.profile_hash)
      errors.push(`${currentStrategy} provider profile hash does not match its route binding`);
  }
  ...
  if (reviewerRegistryBinding && reviewerRegistryBinding.profile_hash !== manifest.reviewer.profile_hash)
    errors.push('reviewer provider profile hash does not match its reviewer binding');
  ...
  for (const currentCase of manifest.cases) {
    ...
    if (currentCase.validation_surface_hash !== manifest.baseline.validation_surface_hash)
      errors.push(`case validation surface differs from baseline: ${currentCase.case_id}`);
    if (currentCase.oracle.kind === 'NONE' && manifest.corpus_policy.provenance === 'historical_commits')
      errors.push(`historical corpus case lacks an oracle: ${currentCase.case_id}`);
  }
  errors.push(...scheduleErrors(manifest));
  if (manifest.corpus_policy.solution_diff_available_to_workers || manifest.corpus_policy.oracle_available_to_workers)
    errors.push('worker corpus projection must not expose solution or oracle data');
  if (manifest.reviewer.sees_executor_narrative || manifest.reviewer.sees_other_route_result)
    errors.push('reviewer must not receive executor narrative or the other route result');
  if (
    manifest.authority.routing_decision !== 'REPORT_ONLY' ||
    manifest.authority.publication_mode !== 'MANUAL_ONLY' ||
    manifest.authority.auto_route_promotion ||
    manifest.authority.auto_push ||
    manifest.authority.auto_merge ||
    manifest.authority.auto_deploy
  ) {
    errors.push('dogfood authority must remain report-only with manual publication');
  }
  if (manifest.manifest_hash !== hashCanonical(omitHash(manifest, 'manifest_hash')))
...
function canonicalBindingRegistry(
  bindings: DogfoodRunRecordV1['provider_cost_evidence']['binding_registry'],
): readonly DogfoodRunRecordV1['provider_cost_evidence']['binding_registry'][number][] {
  return [...bindings].sort((left, right) => compareCodeUnits(left.binding_ref, right.binding_ref));
}

function canonicalUsageEventBindings(
  bindings: DogfoodRunRecordV1['provider_cost_evidence']['usage_event_bindings'],
): readonly DogfoodRunRecordV1['provider_cost_evidence']['usage_event_bindings'][number][] {
...
function costErrors(manifest: DogfoodManifestV1, record: DogfoodRunRecordV1): string[] {
  ...
  if (snapshot.pricing_snapshot_hash !== hashCanonical(snapshotContent))
    errors.push('provider pricing snapshot hash does not match its content');
  if (snapshot.pricing_snapshot_hash !== policy.conversion_policy_hash)
    errors.push('provider pricing snapshot does not match the frozen conversion policy');
  if (snapshot.currency !== policy.reporting_currency)
    errors.push('provider pricing snapshot currency does not match the frozen reporting currency');
  ...
  if (providerEvidence.usage_ledger_hash !== hashCanonical({ usage: canonicalUsage, usage_event_bindings: canonicalUsageEventRefs }))
    errors.push('provider usage ledger hash does not match canonical usage and event references');
  if (providerEvidence.binding_registry_hash !== hashCanonical(canonicalBindingRegistry(providerEvidence.binding_registry)))
    errors.push('provider binding registry hash does not match canonical registry content');
  if (providerEvidence.binding_registry_hash !== topology.binding_registry_hash)
    errors.push('provider binding registry does not match the frozen provider usage topology');
  ...
  if (registryBindingRefs.size !== providerEvidence.binding_registry.length)
    errors.push('provider binding registry references must be unique');
  if (
    registryBindingRefs.size !== allowedBindingRefs.size ||
    [...allowedBindingRefs].some((bindingRef) => !registryBindingRefs.has(bindingRef))
  )
    errors.push('provider binding registry does not match the frozen provider usage registry');
  const tariffBindingRefs = new Set(snapshot.tariffs.map((tariff) => tariff.binding_ref));
  if (
    tariffBindingRefs.size !== registryBindingRefs.size ||
    [...registryBindingRefs].some((bindingRef) => !tariffBindingRefs.has(bindingRef))
  )
    errors.push('provider pricing tariffs do not match the provider binding registry');
  ...
  for (const eventRef of providerEvidence.usage_event_bindings) {
    ...
    if (!record.evidence_hashes.includes(eventRef.event_hash))
      errors.push(`provider usage event ${eventRef.event_id} is not included in the run evidence hashes`);
  }
  if (eventRefsByUsageId.size !== usageIds.size || [...usageIds].some((usageId) => !eventRefsByUsageId.has(usageId)))
    errors.push('provider usage event references must cover every usage entry exactly once');
  const allowedUsageRefs = (role: UsageRecordedV3['role']): readonly string[] =>
    role === 'executor' ? [topology.roles.executor[record.strategy]] : topology.roles[role].allowed_binding_refs;
  ...
  for (const usage of providerEvidence.usage) {
    ...
    if (!allowedUsageRefs(usage.role).includes(usage.binding_ref))
      errors.push(`provider ${usage.role} usage does not match the frozen role binding: ${usage.binding_ref}`);
  }
  for (const requiredRole of topology.required_usage_roles[record.strategy])
    if (!usageRoles.has(requiredRole)) errors.push(`provider usage evidence is missing required ${record.strategy} role: ${requiredRole}`);
  ...
  if (aggregate) {
    ...
    if (aggregate.cost_observed.value !== record.observed_cost_micro_units)
      errors.push('observed_cost_micro_units does not match reproduced provider cost');
    const frontierUsageCalls = aggregate.priced_usage.filter(
      (usage) =>
        providerEvidence.binding_registry.find((binding) => binding.binding_ref === usage.binding_ref)?.capability_class === 'strong',
    ).length;
  ...
  else if (record.human_intervention_cost_micro_units !== expectedHumanCost)
    errors.push('human_intervention_cost_micro_units does not match the frozen human rate');
  ...
  else if (record.total_cost_to_accepted_result_micro_units !== expectedTotal)
    errors.push('total_cost_to_accepted_result_micro_units does not match observed and human cost');
  return errors;
}
...
function semanticErrors(record: DogfoodRunRecordV1): string[] {
  ...
  if (record.human_intervention_seconds > 0 && record.human_interventions === 0)
    errors.push('positive human intervention time requires at least one human intervention');
  ...
  if (record.post_acceptance_defects.some((defect) => defect.severity === 'critical') && !record.false_acceptance)
    errors.push('critical post-acceptance defects require critical false acceptance');
  return errors;
}

type DerivedDogfoodStopCondition = Extract<
  DogfoodStopEventV1['stop_condition'],
  'CRITICAL_FALSE_ACCEPTANCE' | 'CROSS_RUN_CONTAMINATION' | 'UNRECONSTRUCTABLE_EVIDENCE'
>;
...
function derivedStopCondition(record: DogfoodRunRecordV1): DerivedDogfoodStopCondition | null {
  ...
  if (record.false_acceptance && record.post_acceptance_defects.some((defect) => defect.severity === 'critical'))
    return 'CRITICAL_FALSE_ACCEPTANCE';
...
export function freezeDogfoodManifestV1(input: DogfoodManifestInputV1): DogfoodManifestV1 {
  const parsedInput = dogfoodManifestV1Schema
    .omit({ schema_version: true, manifest_hash: true, schedule_hash: true, schedule: true })
    .parse(input);
...
export function verifyDogfoodManifestV1(manifest: DogfoodManifestV1): DogfoodManifestVerificationV1 {
  const errors: string[] = [];
  try {
    loadDogfoodManifestV1(manifest);
  } catch (error) {
...
export function verifyDogfoodStopEventV1(
  manifest: DogfoodManifestV1,
  event: DogfoodStopEventV1,
  triggeringRun: DogfoodRunRecordV1 | null = null,
): DogfoodRunRecordVerificationV1 {
  const errors: string[] = [];
  try {
    loadDogfoodStopEventV1(event);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
  if (errors.length > 0) return { ok: false, errors };
  if (event.stop_event_hash !== hashCanonical(omitHash(event, 'stop_event_hash')))
    errors.push('stop event hash does not match canonical event content');
  if (event.experiment_id !== manifest.experiment_id || event.manifest_hash !== manifest.manifest_hash)
    errors.push('stop event is bound to a different experiment manifest');
  ...
  if (triggeringRun.schedule_ordinal !== event.last_completed_schedule_ordinal)
    errors.push('stop event triggering run does not match the last completed ordinal');
  if (!event.evidence_hashes.includes(triggeringRun.record_hash))
    errors.push('stop event evidence must include the triggering run record hash');
  ...
  if (observedAt !== null && completedAt !== null && observedAt < completedAt)
    errors.push('stop event must be observed after the triggering run completed');
  if (event.stop_condition === 'CRITICAL_FALSE_ACCEPTANCE') {
    ...
    if (!triggeringRun.post_acceptance_defects.some((defect) => defect.severity === 'critical'))
      errors.push('critical false acceptance stop requires a critical post-acceptance defect on the triggering run');
  }
  if (event.stop_condition === 'CROSS_RUN_CONTAMINATION' && !triggeringRun.cross_run_contamination)
    errors.push('cross-run contamination stop requires cross_run_contamination=true on the triggering run');
  if (event.stop_condition === 'UNRECONSTRUCTABLE_EVIDENCE' && triggeringRun.evidence_reconstructible)
    errors.push('unreconstructable evidence stop requires evidence_reconstructible=false on the triggering run');
  return { ok: errors.length === 0, errors };
}
...
export function verifyDogfoodRunRecordV1(manifest: DogfoodManifestV1, record: DogfoodRunRecordV1): DogfoodRunRecordVerificationV1 {
  const errors: string[] = [];
  try {
    loadDogfoodRunRecordV1(record);
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
  if (errors.length > 0) return { ok: false, errors };
  if (record.record_hash !== hashCanonical(omitHash(record, 'record_hash')))
    errors.push('record hash does not match canonical record content');
  if (record.experiment_id !== manifest.experiment_id || record.manifest_hash !== manifest.manifest_hash)
    errors.push('record is bound to a different experiment manifest');
  ...
  if (scheduled && (scheduled.case_id !== record.case_id || scheduled.strategy !== record.strategy))
    errors.push('record does not match its frozen schedule entry');
  const routeBinding = manifest.route_bindings.find((value) => value.strategy === record.strategy);
  if (
    !routeBinding ||
    record.binding_ref !== routeBinding.binding_ref ||
    record.binding_hash !== routeBinding.binding_hash ||
    record.qualification_hash !== routeBinding.qualification_hash
  )
    errors.push('record binding does not match the frozen route binding');
  for (const [field, expected] of Object.entries({
  ...
  if (record.publication_state !== 'MANUAL_PENDING' && record.publication_state !== 'NOT_REQUESTED')
    errors.push('record publication state is not manual');
  return { ok: errors.length === 0, errors };
}

export function verifyDogfoodRunSetV1(
  manifest: DogfoodManifestV1,
  records: readonly unknown[],
  stopEvent: DogfoodStopEventV1 | null = null,
): DogfoodRunSetVerificationV1 {
  const errors: string[] = [];
  ...
  const expectedLastOrdinal = stopEvent
    ? Math.min(stopEvent.last_completed_schedule_ordinal, manifest.schedule.length)
  ...
  for (const record of parsedRecords)
    if (!expectedOrdinals.has(record.schedule_ordinal))
      errors.push(`record occurs after the frozen stop ordinal: ${record.schedule_ordinal}`);
  const orderedRecords = [...parsedRecords].sort((left, right) => left.schedule_ordinal - right.schedule_ordinal);
  const firstDerivedStop =
    orderedRecords
  ...
  if (firstDerivedStop) {
    if (stopEvent === null) {
      errors.push(
        `derived hard stop ${firstDerivedStop.stopCondition} at ordinal ${firstDerivedStop.record.schedule_ordinal} requires a hash-bound stop event; run set cannot be COMPLETE`,
      );
    } else {
      if (stopEvent.last_completed_schedule_ordinal !== firstDerivedStop.record.schedule_ordinal)
        errors.push('stop event must target the first observable derived hard-stop ordinal');
      if (stopEvent.stop_condition !== firstDerivedStop.stopCondition)
        errors.push('stop event must use the first observable derived hard-stop condition');
      if (stopEvent.triggering_run_id !== firstDerivedStop.record.run_id)
        errors.push('stop event must identify the first observable derived hard-stop run');
    }
  }
  if (manifest.run_policy.execution_mode === 'STRICT_SERIAL') {
    for (let index = 1; index < orderedRecords.length; index += 1) {
      ...
      if (prior !== null && current !== null && current < prior)
        errors.push(
          `strict serial execution order violated between ordinals ${orderedRecords[index - 1]!.schedule_ordinal} and ${orderedRecords[index]!.schedule_ordinal}`,
        );
    }
  }
  ...
  if (!stopEvent && [...expectedPairs].some((pair) => !actualPairs.has(pair)))
    errors.push('run record set must contain each case and route exactly once');
  if (stopEvent) {
    const triggeringRun = parsedRecords.find((record) => record.run_id === stopEvent.triggering_run_id);
    if (stopEvent.last_completed_schedule_ordinal > 0 && !triggeringRun)
      errors.push('stop event triggering run is missing from the stopped prefix');
