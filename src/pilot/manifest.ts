...
const equivalentFields = [
  'case_fingerprint', 'contract_hash', 'base_revision', 'clean_tree_hash', 'fixtures_hash', 'validation_surface',
  'complexity_class', 'risk_class', 'changed_line_band',
] as const;
...
function comparativeErrors(blocks: readonly PilotManifestV3['blocks'][number][]): string[] {
  ...
  for (const block of blocks) {
    ...
    if (classification.risk_class === 'restricted') errors.push(`comparative_eligible block cannot be restricted: ${classification.block_id}`);
    if (classification.exclusion_reason !== null) errors.push(`comparative_eligible block cannot have an exclusion reason: ${classification.block_id}`);
...
export function assignArms(input: FrozenAssignmentInputV3): ArmAssignmentV3[] {
  ...
  for (const [tripletId, members] of [...comparativeTriplets(input).entries()].sort(([left], [right]) => left.localeCompare(right))) {
    ...
    const rankedArms = [...arms].sort((left, right) => hashCanonical({
...
function manifestErrors(manifest: PilotManifestV3): string[] {
  ...
  for (const [blockId, assignment] of assignments) {
    ...
    if (block.selected_executor_capability_initial !== expected.initial
      || block.selected_executor_capability_final_expected !== expected.final) {
      const required = expected.initial === expected.final
  ...
  else if (reviewer.capability_class !== manifest.routing_reviewer_capability) errors.push('routing reviewer capability must match its binding');
  const tariffBindings = new Set<string>();
  if (!Number.isSafeInteger(manifest.pricing_snapshot.unit_scale) || manifest.pricing_snapshot.unit_scale <= 0) errors.push('pricing snapshot unit scale must be a positive safe integer');
  for (const tariff of manifest.pricing_snapshot.tariffs) {
    ...
    for (const value of [tariff.input_token_micro_units_per_token, tariff.output_token_micro_units_per_token, tariff.cached_input_token_micro_units_per_token, tariff.reasoning_token_micro_units_per_token]) {
      if (value !== null && (!Number.isSafeInteger(value) || value < 0)) errors.push(`pricing tariff contains unsafe integer: ${tariff.binding_ref}`);
    }
  }
  for (const binding of registry.keys()) if (!tariffBindings.has(binding)) errors.push(`pricing snapshot lacks tariff for registered binding: ${binding}`);
