...
const postAcceptanceWindowSchema = z.object({
...
const manifestBlockBase = z.object({
  block_id: identifier,
  task_id: identifier,
...
export const pilotManifestV3Schema = z.object({
  pilot_id: identifier,
...
const eventBase = {
  schema_version: z.literal(3), event_id: identifier, pilot_id: identifier, manifest_hash: hash,
  task_id: identifier, block_id: identifier, matching_stratum: identifier, pair_or_triplet_id: identifier,
  case_fingerprint: hash, pilot_arm: pilotArm.nullable(), sequence_number: positiveInteger,
  occurred_at: timestamp, recorded_at: timestamp, producer_id: identifier,
};
...
const executionPayload = z.object({
  attempt_id: identifier, attempt_number: positiveInteger,
  attempt_kind: z.enum(['IMPLEMENTATION', 'REPAIR_1', 'FINAL_EXECUTION']),
  executor_capability: capabilityClass, executor_binding_ref: identifier, executor_session_id: identifier,
  input_revision: hash, output_revision: hash, output_tree_hash: hash, output_diff_hash: hash,
  canonical_tree_algorithm_version: identifier, volatile_paths_policy_hash: hash,
  tree_reproduced: z.boolean(), tree_reproduction_evidence_hash: hash,
  changed_lines_production: nonnegativeInteger, changed_lines_tests: nonnegativeInteger, changed_lines_docs: nonnegativeInteger,
  outcome: z.literal('COMPLETED'), started_monotonic_ms: nonnegativeInteger,
  finished_monotonic_ms: nonnegativeInteger, duration_ms: nonnegativeInteger,
}).strict();

const findingSchema = z.object({
  finding_id: identifier, severity, material: z.boolean(), category_code: identifier,
  status: z.enum(['OPEN', 'RESOLVED']), evidence_hashes: boundedArray(hash).min(1),
}).strict();

const reviewPayload = z.object({
  review_id: identifier, review_round: positiveInteger, reviewer_binding_ref: identifier, reviewer_session_id: identifier,
  reviewed_attempt_id: identifier, executor_session_id_reviewed: identifier, review_input_diff_hash: hash,
  previous_review_boundary_hash: hash.nullable(), review_boundary_hash: hash,
  review_boundary_from_revision: hash, review_boundary_to_revision: hash,
  unresolved_finding_ids: boundedArray(identifier), validation_evidence_hashes: boundedArray(hash), bounded_context_hashes: boundedArray(hash),
  additional_context_requests: boundedArray(z.enum(['VALIDATION_EVIDENCE', 'BOUNDARY_HASH', 'USAGE_RECORD'])),
  material_findings: boundedArray(findingSchema), non_material_findings: boundedArray(findingSchema),
  decision: z.enum(['ACCEPT', 'REJECT']), started_monotonic_ms: nonnegativeInteger,
  finished_monotonic_ms: nonnegativeInteger, duration_ms: nonnegativeInteger,
}).strict();
...
const usageObservedCostShapes = [
  ...
  { cost_provenance: z.literal('TARIFF_REPRODUCED'), cost_observed: nonnegativeInteger, cost_estimated: z.null(), input_tokens_observed: nonnegativeInteger, output_tokens_observed: nonnegativeInteger },
  { cost_provenance: z.literal('AUTHORITATIVE_BILL'), provider_usage_id: identifier, cost_observed: nonnegativeInteger, cost_estimated: z.null() },
];
const usageEstimatorShapes = [
  { input_tokens_estimated: z.null(), output_tokens_estimated: z.null(), cached_input_tokens_estimated: z.null(), reasoning_tokens_estimated: z.null(), token_estimator_id: z.null(), token_estimator_version: z.null() },
  { input_tokens_estimated: nonnegativeInteger, token_estimator_id: identifier, token_estimator_version: identifier },
  { input_tokens_estimated: z.null(), output_tokens_estimated: nonnegativeInteger, token_estimator_id: identifier, token_estimator_version: identifier },
  { input_tokens_estimated: z.null(), output_tokens_estimated: z.null(), cached_input_tokens_estimated: nonnegativeInteger, token_estimator_id: identifier, token_estimator_version: identifier },
  { input_tokens_estimated: z.null(), output_tokens_estimated: z.null(), cached_input_tokens_estimated: z.null(), reasoning_tokens_estimated: nonnegativeInteger, token_estimator_id: identifier, token_estimator_version: identifier },
];
...
type PilotEventBaseV3 = {
  schema_version: 3; event_id: string; pilot_id: string; manifest_hash: string; task_id: string; block_id: string;
  matching_stratum: string; pair_or_triplet_id: string; case_fingerprint: string; pilot_arm: PilotArmV3 | null;
  sequence_number: number; occurred_at: string; recorded_at: string; producer_id: string;
};
type PilotEventPayloadMapV3 = {
  ...
  ISOLATION_ATTESTED: { workspace_instance_id: string; base_revision: string; clean_tree_hash: string; isolation_status: 'CLEAN' | 'CONTAMINATED'; observed_tree_hash: string; isolation_policy_version: string; attestor_id: string; evidence_hash: string };
  EXECUTION_STARTED: { attempt_id: string; attempt_number: number; attempt_kind: 'IMPLEMENTATION' | 'REPAIR_1' | 'FINAL_EXECUTION'; executor_capability: 'cheap' | 'strong'; executor_binding_ref: string; executor_session_id: string; input_revision: string; started_monotonic_ms: number };
  EXECUTION_COMPLETED: z.infer<typeof executionPayload>;
  REVIEW_STARTED: { review_id: string; review_round: number; reviewer_binding_ref: string; reviewer_session_id: string; reviewed_attempt_id: string; executor_session_id_reviewed: string; started_monotonic_ms: number };
  REVIEW_COMPLETED: z.infer<typeof reviewPayload>;
  VALIDATION_RECORDED: { validation_id: string; attempt_id: string; validation_surface: string[]; passed: boolean; tests_failing: number; tests_passing: number; evidence_hashes: string[] };
  ORCHESTRATOR_OPERATION_RECORDED: { orchestrator_operation_id: string; attempt_number: number; binding_ref: string; evidence_hash: string };
  USAGE_RECORDED: UsagePayloadV3;
  PARENT_REWORK_RECORDED: { review_id: string; attempt_id: string; files_production: string[]; files_tests: string[]; files_docs: string[]; lines_production: number; lines_tests: number; lines_docs: number; diff_hash: string; actor_role: 'orchestrator' | 'reviewer' | 'human'; reason_code: string };
  ...
  ESCALATION_DECIDED: { rejected_review_event_id: string; escalation_reason: string; target_binding_ref: string; target_capability: 'strong'; decision_policy_version: string };
  POST_ACCEPT_DEFECT_RECORDED: { defect_id: string; severity: 'low' | 'medium' | 'high' | 'critical'; material: boolean; discovered_at: string; evidence_id: string; affected_revision: string; accepted_review_id: string; category_code: string };
...
type EventFor<K extends keyof PilotEventPayloadMapV3> = PilotEventBaseV3 & { event_type: K; payload: PilotEventPayloadMapV3[K] } & (
...
const pilotEventVariants = [
  ...
  ...event('ISOLATION_ATTESTED', z.object({ workspace_instance_id: identifier, base_revision: hash, clean_tree_hash: hash, isolation_status: z.enum(['CLEAN', 'CONTAMINATED']), observed_tree_hash: hash, isolation_policy_version: identifier, attestor_id: identifier, evidence_hash: hash }).strict()),
  ...event('EXECUTION_STARTED', z.object({ attempt_id: identifier, attempt_number: positiveInteger, attempt_kind: z.enum(['IMPLEMENTATION', 'REPAIR_1', 'FINAL_EXECUTION']), executor_capability: capabilityClass, executor_binding_ref: identifier, executor_session_id: identifier, input_revision: hash, started_monotonic_ms: nonnegativeInteger }).strict()),
  ...event('EXECUTION_COMPLETED', executionPayload),
  ...event('REVIEW_STARTED', z.object({ review_id: identifier, review_round: positiveInteger, reviewer_binding_ref: identifier, reviewer_session_id: identifier, reviewed_attempt_id: identifier, executor_session_id_reviewed: identifier, started_monotonic_ms: nonnegativeInteger }).strict()),
  ...
  ...event('BLOCK_BLOCKED', z.object({ cause: z.enum(['EXTERNAL', 'ENVIRONMENTAL']), reason_code: identifier, evidence_hash: hash }).strict()),
  ...event('ESCALATION_DECIDED', z.object({ rejected_review_event_id: identifier, escalation_reason: identifier, target_binding_ref: identifier, target_capability: z.literal('strong'), decision_policy_version: identifier }).strict()),
  ...event('POST_ACCEPT_DEFECT_RECORDED', z.object({ defect_id: identifier, severity, material: z.boolean(), discovered_at: timestamp, evidence_id: identifier, affected_revision: hash, accepted_review_id: identifier, category_code: identifier }).strict()),
  ...event('EVENT_INVALIDATED', z.object({ invalidated_event_id: identifier, expected_event_content_hash: hash, reason_code: identifier }).strict(), false),
...
const strongTokenAggregateSchema = z.object({
...
const postAcceptDefectSchema = z.object({
  defect_id: identifier, severity, material: z.boolean(), discovered_at: timestamp,
  evidence_id: identifier, affected_revision: hash, category_code: identifier,
}).strict();

const observationBaseShape = {
  schema_version: z.literal(3), pilot_id: identifier, manifest_hash: hash, task_id: identifier, block_id: identifier,
  matching_stratum: identifier, pair_or_triplet_id: identifier, case_fingerprint: hash, pilot_arm: pilotArm.nullable(),
  complexity_class: complexityClass, risk_class: riskClass, changed_line_band: identifier,
  cheap_eligible: z.boolean(), comparative_eligible: z.boolean(),
  state: z.enum(['PLANNED', 'ASSIGNED', 'READY_1', 'EXECUTING_1', 'READY_REVIEW_1', 'REVIEWING_1', 'READY_2', 'EXECUTING_2', 'READY_REVIEW_2', 'REVIEWING_2', 'ESCALATION_REQUIRED', 'READY_3', 'EXECUTING_3', 'READY_FINAL_REVIEW', 'FINAL_REVIEWING', 'READY_ACCEPT', 'READY_FAIL', 'ACCEPTED', 'FAILED', 'BLOCKED', 'INVALID']),
  valid_history: z.boolean(), invalid_reason_codes: boundedArray(identifier), executor_binding_initial: identifier.nullable(), executor_binding_final: identifier.nullable(), reviewer_binding_refs: boundedArray(identifier),
  execution_attempts: nonnegativeInteger.max(3), repair_rounds: nonnegativeInteger.max(1), escalated: z.boolean(), escalation_reason: identifier.nullable(), first_pass_accept: z.boolean(), accept_after_one_repair: z.boolean(), final_accepted: z.boolean(),
  tests_initially_failing: nonnegativeInteger, tests_finally_passing: nonnegativeInteger, review_findings_material: nonnegativeInteger, review_findings_non_material: nonnegativeInteger,
  parent_rework_files: z.object({ production: nonnegativeInteger, tests: nonnegativeInteger, docs: nonnegativeInteger }).strict(), parent_rework_lines_production: nonnegativeInteger, parent_rework_lines_tests: nonnegativeInteger, parent_rework_lines_docs: nonnegativeInteger,
  changed_lines_production: nonnegativeInteger, changed_lines_tests: nonnegativeInteger, changed_lines_docs: nonnegativeInteger,
  orchestrator_usage: usageSummarySchema, executor_usage: usageSummarySchema, reviewer_usage: usageSummarySchema, total_usage: usageSummarySchema,
  cost_observed: nonnegativeInteger.nullable(), cost_estimated: nonnegativeInteger.nullable(), cost_observed_completeness: rate, cost_estimated_completeness: rate,
  strong_tokens_observed: strongTokenAggregateSchema, strong_tokens_estimated: strongTokenAggregateSchema,
  wall_time_seconds: finiteNonnegativeNumber.nullable(), executor_time_seconds: finiteNonnegativeNumber.nullable(), review_time_seconds: finiteNonnegativeNumber.nullable(),
  blocked_cause: z.enum(['EXTERNAL', 'ENVIRONMENTAL']).nullable(), blocked_reason_code: identifier.nullable(),
  post_acceptance_window_closed: z.boolean(), accepted_at: timestamp.nullable(), window_opens_at: timestamp.nullable(), window_closes_at: timestamp.nullable(), post_accept_defects: boundedArray(postAcceptDefectSchema), post_accept_defects_count: nonnegativeInteger, post_accept_max_severity: severity.nullable(), late_quality_evidence_count: nonnegativeInteger, quality_warnings: boundedArray(identifier), final_outcome: z.enum(['ACCEPTED', 'FAILED', 'BLOCKED', 'INVALID']),
};
...
const acceptedObservationOutcome = {
  final_outcome: z.literal('ACCEPTED'), final_accepted: z.literal(true),
  valid_history: z.literal(true), state: z.literal('ACCEPTED'), blocked_cause: z.null(), blocked_reason_code: z.null(),
};
...
const nonAcceptedObservationOutcome = {
  final_outcome: z.enum(['FAILED', 'INVALID']), final_accepted: z.literal(false),
  first_pass_accept: z.literal(false), accept_after_one_repair: z.literal(false), blocked_cause: z.null(), blocked_reason_code: z.null(),
};
const blockedObservationOutcome = {
  final_outcome: z.literal('BLOCKED'), final_accepted: z.literal(false), first_pass_accept: z.literal(false), accept_after_one_repair: z.literal(false),
  blocked_cause: z.enum(['EXTERNAL', 'ENVIRONMENTAL']), blocked_reason_code: identifier,
};
...
const observationVariants = [
  ...observationEscalationShapes.flatMap(escalation => acceptedObservationCumulativeShapes.flatMap(cumulative => acceptedObservationWindowShapes.map(window => z.object({ ...observationBaseShape, ...acceptedObservationOutcome, ...escalation, ...cumulative, ...window }).strict()))),
  ...observationEscalationShapes.flatMap(escalation => nonAcceptedObservationWindowShapes.map(window => z.object({ ...observationBaseShape, ...nonAcceptedObservationOutcome, ...escalation, ...window }).strict())),
  ...observationEscalationShapes.flatMap(escalation => nonAcceptedObservationWindowShapes.map(window => z.object({ ...observationBaseShape, ...blockedObservationOutcome, ...escalation, ...window }).strict())),
];

export const pilotBlockObservationV3Schema = z.union(observationVariants as unknown as [z.ZodType, z.ZodType, ...z.ZodType[]]).superRefine((value, context) => {
  const observation = value as PilotBlockObservationShapeV3;
  if (observation.pilot_arm === null && observation.comparative_eligible) context.addIssue({ code: 'custom', message: 'null pilot arm requires non-comparative observation' });
}) as z.ZodType<PilotBlockObservationShapeV3>;

function gateThresholdsSchema(minimumBlocksPerArm: 10 | 20 | 30) {
  return z.object({
    minimum_blocks_per_arm: z.literal(minimumBlocksPerArm), material_improvement_rate: rate,
    economic_rejection_rate: rate, max_parent_rework_block_rate: rate, max_parent_rework_production_line_share: rate,
    max_escaped_material_defects: nonnegativeInteger, max_escaped_high_defects: nonnegativeInteger, max_escaped_critical_defects: nonnegativeInteger,
    min_observed_cost_completeness: positiveRate, min_observed_strong_token_completeness: positiveRate, min_stratum_triplets_for_promotion: positiveInteger.max(30),
    confidence_level: z.number().gt(0).lt(1), interval_algorithm_version: z.literal('paired-bootstrap-sha256-counter-v1'), resampling_iterations: resamplingIterations,
  }).strict();
}
...
const routingGateBase = {
  schema_version: z.literal(3), gate_policy_id: identifier, pilot_id: identifier,
  manifest_hash: hash, evaluated_at: timestamp, resampling_seed: identifier,
...
const resourceMetricSampleSchema = z.object({
...
const rateMetricSampleSchema = z.object({
  numerator: nonnegativeInteger,
  denominator: nonnegativeInteger,
  value: rate.nullable(),
  confidence_interval: rateIntervalSchema.nullable(),
}).strict();
const countMetricSampleSchema = z.object({
  numerator: nonnegativeInteger,
  denominator: nonnegativeInteger,
  value: nonnegativeNumber.nullable(),
  confidence_interval: nonnegativeIntervalSchema.nullable(),
}).strict();
const armMetricSchema = z.object({
...
const armCountsSchema = z.object({
...
const armCompletenessSchema = z.object({
...
const pairedQualityCounts = {
  baseline_successes: nonnegativeInteger, candidate_successes: nonnegativeInteger,
  both_success: nonnegativeInteger, baseline_only_success: nonnegativeInteger,
  candidate_only_success: nonnegativeInteger, neither_success: nonnegativeInteger,
};
const pairedQualitySchema = z.union([
  ...
  z.object({ ...pairedQualityCounts, denominator: positiveInteger, difference: z.number().min(-1).max(1), confidence_interval: signedRateIntervalSchema }).strict(),
]);
const pairedMetricSchema = z.object({
  baseline_value: nonnegativeNumber.nullable(), candidate_value: nonnegativeNumber.nullable(),
  relative_improvement: z.number().nullable(), confidence_interval: signedIntervalSchema.nullable(),
}).strict();
const evaluationMetricsSchema = z.object({
  by_arm: armMetricsSchema,
  paired_comparisons: z.object({
...
const resourceMeasureSchema = z.object({
...
const resourceTotalsSchema = z.object({
...
const evaluationDenominatorsSchema = z.object({
...
const strongTokenCompletenessSchema = z.object({
...
const evaluationCompletenessSchema = z.object({
...
const intervalMetadataSchema = z.object({
  confidence_level: z.number().gt(0).lt(1),
...
const efficiencyBranchesSchema = z.tuple([
  efficiencyBranchRecord('observed_cost'),
  efficiencyBranchRecord('observed_strong_tokens'),
]);
const stratumEvaluationSchema = z.object({
  matching_stratum: identifier,
  candidate_triplets: nonnegativeInteger,
  admitted_triplets: nonnegativeInteger,
...
const evaluationReportBase = {
  schema_version: z.literal(3), evaluation_id: identifier, evaluation_version: positiveInteger, pilot_id: identifier,
  manifest_hash: hash, evaluated_at: timestamp,
  observation_set_hash: hash, decision_input_hash: hash, quality_evidence_hash: hash, quality_evidence_count: nonnegativeInteger,
  gate_policy_hash: hash, late_quality_evidence_count: nonnegativeInteger,
  not_validated_strata: boundedArray(identifier), reasons: boundedArray(identifier).min(1),
  metrics: evaluationMetricsSchema, exclusions: boundedArray(evaluationExclusionSchema),
  efficiency_branches: efficiencyBranchesSchema,
  strata: boundedArray(stratumEvaluationSchema),
  operational_totals: operationalTotalsSchema,
  denominators: evaluationDenominatorsSchema, completeness: evaluationCompletenessSchema,
  interval_metadata: intervalMetadataSchema, warnings: boundedArray(identifier),
};
...
const reportVariants = reportDecisionShapes.flatMap(decision => reportSupersessionShapes.map(supersession => (
  z.object({ ...evaluationReportBase, ...decision, ...supersession }).strict()
)));
...
export const pilotEvaluationReportV3Schema: z.ZodType<PilotEvaluationReportV3> = z.union(
  reportVariants as unknown as [z.ZodType<PilotEvaluationReportV3>, z.ZodType<PilotEvaluationReportV3>, ...z.ZodType<PilotEvaluationReportV3>[]],
);
