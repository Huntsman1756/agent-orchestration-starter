...
import { computePilotMetrics, computePilotMetricsForTripletIds, type PilotMetricTripletV3, type PilotMetricsV3 } from './metrics.js';
...
const arms: readonly Arm[] = ['A_STRONG_BASELINE', 'B_CHEAP_NO_EARLY_ESCALATION', 'C_ADAPTIVE_EARLY_ESCALATION'];
...
export function deterministicBootstrapIndices(input: DeterministicBootstrapIndicesInputV3): readonly (readonly number[])[] {
...
function percentileInterval(statistics: readonly (number | null)[], confidenceLevel: number): Interval | null {
...
function bootstrapInterval(
  ...
  return percentileInterval(
    indices.map((sample) => statistic(sample.map((index) => ordered[index]))),
    gate.thresholds.confidence_level,
  );
}
...
function pairedDifference(
  ...
  const difference = sample.reduce(
    (sum, triplet) =>
      sum + Number(predicate(triplet.members.C_ADAPTIVE_EARLY_ESCALATION)) - Number(predicate(triplet.members.A_STRONG_BASELINE)),
    0,
  );
...
function resourceValue(observation: PilotBlockObservationV3, branch: ResourceBranch): number | null {
  return branch === 'observed_cost' ? observation.cost_observed : observation.strong_tokens_observed.total.value;
}
...
function resourceAverages(sample: readonly PilotMetricTripletV3[], branch: ResourceBranch): { baseline: number; candidate: number } | null {
  ...
  for (const triplet of sample) {
    ...
    if (!Number.isSafeInteger(baselineValue) || !Number.isSafeInteger(candidateValue))
...
function eligibleTriplets(metrics: PilotMetricsV3, branch: ResourceBranch): PilotMetricTripletV3[] {
  const ids = new Set(
    branch === 'observed_cost' ? metrics.populations.observed_cost_triplet_ids : metrics.populations.observed_strong_token_triplet_ids,
  );
  return metrics.populations.core_triplets.filter((triplet) => ids.has(triplet.pair_or_triplet_id));
}

function branchRecord(manifest: PilotManifestV3, gate: PilotRoutingGateV3, metrics: PilotMetricsV3, branch: ResourceBranch): Branch {
  ...
  const minimumCompleteness =
    branch === 'observed_cost' ? gate.thresholds.min_observed_cost_completeness : gate.thresholds.min_observed_strong_token_completeness;
  let status: Branch['status'];
  let reason: string;
  if (completeness !== null && completeness >= minimumCompleteness && zeroBaselineRegression) {
    status = 'UNUSABLE';
    reason = 'zero_baseline_regression';
  } else if (core === 0 || completeness === null || completeness < minimumCompleteness || point === null) {
    status = 'UNUSABLE';
    reason = branch === 'observed_cost' ? 'observed_cost_incomplete' : 'incomplete_observed_evidence';
  } else if (
    point >= gate.thresholds.material_improvement_rate &&
    interval !== null &&
    interval.lower >= gate.thresholds.material_improvement_rate
  ) {
    status = 'PASS';
    reason = branch === 'observed_cost' ? 'material_cost_improvement' : 'material_strong_token_improvement';
  } else if (interval !== null && interval.upper < gate.thresholds.material_improvement_rate) {
    status = 'FAIL_POINT';
    reason = 'material_improvement_ruled_out';
  } else {
    status = 'AMBIGUOUS';
    reason = 'decision_remains_ambiguous';
  }
  return {
    branch,
    status,
...
function validateGate(manifest: PilotManifestV3, gate: PilotRoutingGateV3): void {
  ...
  for (const item of gate.strata_policy) {
    ...
    if (
      !expectedStratum ||
      seen.has(item.matching_stratum) ||
      item.complexity_class !== expectedStratum.complexity_class ||
      item.risk_class !== expectedStratum.risk_class ||
      (item.promotion_eligible && (complexity === 'systemic' || risk === 'high' || risk === 'restricted')) ||
      item.promotion_eligible === (item.exclusion_reason !== null)
    )
      throw new Error('STRATA_POLICY_INVALID');
...
function withIntervals(
  ...
  result.paired_comparisons.final_acceptance.confidence_interval = bootstrapInterval(manifest, gate, core, (sample) =>
    pairedDifference(sample, (observation) => observation.final_accepted),
  )!;
  result.paired_comparisons.final_quality.confidence_interval = bootstrapInterval(manifest, gate, core, (sample) =>
    pairedDifference(sample, qualitySuccess),
  )!;
...
function hardRejectReasons(
  ...
  if (
    globalIntegrityReasons.length > 0 ||
  ...
  if (candidate.escaped_material_defect_rate.numerator > gate.thresholds.max_escaped_material_defects)
    reasons.push('material_post_accept_defect');
  ...
  if (candidate.escaped_critical_defects.numerator > gate.thresholds.max_escaped_critical_defects)
    reasons.push('critical_post_accept_defect');
  if ((candidate.parent_rework_block_rate.value ?? 0) > gate.thresholds.max_parent_rework_block_rate)
    reasons.push('parent_rework_block_rate_above_maximum');
  if ((candidate.parent_rework_production_line_share.value ?? 0) > gate.thresholds.max_parent_rework_production_line_share)
    reasons.push('parent_rework_line_share_above_maximum');
  const wall = paired.wall_time_per_accepted_block;
  if (wall.baseline_value !== null && wall.candidate_value !== null && wall.candidate_value > wall.baseline_value)
    reasons.push('wall_time_above_baseline');
  if (
    branches[0].completeness !== null &&
    branches[0].completeness >= gate.thresholds.min_observed_cost_completeness &&
    ((branches[0].point_improvement !== null && branches[0].point_improvement <= -gate.thresholds.economic_rejection_rate) ||
...
function stratumReports(
  ...
  return stratumNames.map((matchingStratum) => {
    const ids = new Set(
      manifest.blocks
        .filter((block) => selectedIds.has(block.pair_or_triplet_id) && block.matching_stratum === matchingStratum)
        .map((block) => block.pair_or_triplet_id),
    );
    ...
    const acceptance = structuredClone(
      metrics.metrics.paired_comparisons.final_acceptance,
    ) as PilotEvaluationReportV3['strata'][number]['paired_final_acceptance'];
    const quality = structuredClone(
      metrics.metrics.paired_comparisons.final_quality,
    ) as PilotEvaluationReportV3['strata'][number]['paired_final_quality'];
    acceptance.confidence_interval = bootstrapInterval(manifest, gate, metrics.populations.core_triplets, (sample) =>
      pairedDifference(sample, (value) => value.final_accepted),
    )!;
    quality.confidence_interval = bootstrapInterval(manifest, gate, metrics.populations.core_triplets, (sample) =>
      pairedDifference(sample, qualitySuccess),
    )!;
    ...
    if (acceptance.confidence_interval === null || acceptance.confidence_interval.lower < 0)
      reasons.push('stratum_acceptance_not_supported');
...
function validateContext(
  ...
  if (
    !prior ||
    context.evaluation_version !== prior.evaluation_version + 1 ||
    prior.pilot_id !== manifest.pilot_id ||
    prior.manifest_hash !== manifest.manifest_hash
  ) {
  ...
  if ((prior.decision === 'REJECT' || prior.decision === 'PROMOTE_BOUNDED') && gate.stage > prior.stage)
    throw new Error('TERMINAL_DECISION_CANNOT_ADVANCE');
...
export function evaluatePilot(
  ...
  const independentlyPromoted = candidateStrata
    .filter((stratum) => stratum.status === 'PROMOTED')
    .map((stratum) => stratum.matching_stratum);
  ...
  const wallTimeComplete = wallCompleteness.A_STRONG_BASELINE === 1 && wallCompleteness.C_ADAPTIVE_EARLY_ESCALATION === 1;
  if (fullEvidence.late_quality_evidence_count > 0 && context.evaluation_version === 1) warnings.push('LATE_QUALITY_EVIDENCE');
  if (context.prior_report && fullEvidence.late_quality_evidence_count > context.prior_report.late_quality_evidence_count)
    warnings.push('STALE_DECISION');
  ...
  if (hardReasons.length > 0) {
    ...
    decision = 'CONTINUE';
    reasons.push('stage_1_cannot_promote');
  } else if (globallyAmbiguousQuality) {
    decision = gate.stage === 2 ? 'CONTINUE' : 'INCONCLUSIVE';
    reasons.push('decision_remains_ambiguous');
  } else if (branches.some((branch) => branch.status === 'PASS') && !wallTimeComplete) {
    decision = gate.stage === 2 ? 'INSUFFICIENT_EVIDENCE' : 'INCONCLUSIVE';
    reasons.push('wall_time_incomplete');
  } else if (branches.some((branch) => branch.status === 'PASS') && independentlyPromoted.length > 0) {
    decision = 'PROMOTE_BOUNDED';
    promotedStrata = independentlyPromoted;
    reasons.push(
      'bounded_promotion_supported',
      ...branches.filter((branch) => branch.status === 'PASS').flatMap((branch) => branch.reason_codes),
    );
  } else if (branches.some((branch) => branch.status === 'PASS')) {
    decision = gate.stage === 2 ? 'INSUFFICIENT_EVIDENCE' : 'INCONCLUSIVE';
    reasons.push('no_independently_validated_strata');
  } else if (branches.some((branch) => branch.status === 'AMBIGUOUS')) {
    decision = gate.stage === 2 ? 'CONTINUE' : 'INCONCLUSIVE';
    reasons.push('decision_remains_ambiguous');
  } else if (branches.every((branch) => branch.status === 'FAIL_POINT')) {
    decision = 'REJECT';
    reasons.push('material_improvement_ruled_out');
  } else {
    decision = gate.stage === 2 ? 'INSUFFICIENT_EVIDENCE' : 'INCONCLUSIVE';
    reasons.push(
      'insufficient_observed_economic_evidence',
      ...branches.filter((branch) => branch.status === 'UNUSABLE').flatMap((branch) => branch.reason_codes),
    );
  }
  if (
    context.prior_report &&
    gate.stage === context.prior_report.stage &&
    decision === 'PROMOTE_BOUNDED' &&
  ...
  const strata =
    decision === 'PROMOTE_BOUNDED'
      ? candidateStrata
      : candidateStrata.map((stratum) =>
          stratum.status === 'NOT_VALIDATED'
            ? stratum
            : {
                ...stratum,
  ...
  const reportCandidate = {
    ...
    not_validated_strata: strata
      .filter((stratum) => stratum.status === 'NOT_VALIDATED')
      .map((stratum) => stratum.matching_stratum)
  ...
  if (!parsed.success)
    throw new Error(
      `EVALUATION_REPORT_INVALID:${parsed.error.issues.map((issue) => `${issue.path.join('.')}:${issue.message}`).join(';')}`,
    );
...
function validateHistoryTransition(prior: PilotEvaluationReportV3, report: PilotEvaluationReportV3): void {
  if (
    report.pilot_id !== prior.pilot_id ||
    report.manifest_hash !== prior.manifest_hash ||
    ...
    report.expected_superseded_report_hash !== hashCanonical(prior)
  )
    throw new Error('EVALUATION_SUPERSESSION_INVALID');
  ...
  if ((prior.decision === 'REJECT' || prior.decision === 'PROMOTE_BOUNDED') && report.stage > prior.stage)
    throw new Error('TERMINAL_DECISION_CANNOT_ADVANCE');
  ...
  if (
    report.stage === prior.stage &&
    report.decision === 'PROMOTE_BOUNDED' &&
    (prior.decision === 'REJECT' || (prior.stage === 3 && prior.decision !== 'PROMOTE_BOUNDED'))
  )
    throw new Error('TERMINAL_DECISION_CANNOT_PROMOTE');
}

export function appendEvaluation(history: PilotEvaluationHistoryV3, report: PilotEvaluationReportV3): PilotEvaluationHistoryV3 {
  const reportHash = hashCanonical(report);
  for (const existing of history) {
    const sameIdentity =
      existing.evaluation_id === report.evaluation_id ||
  ...
  if (history.length === 0) {
    if (
      report.evaluation_version !== 1 ||
      report.stage !== 1 ||
      report.supersedes_evaluation_id !== null ||
      report.supersedes_evaluation_version !== null ||
      report.expected_superseded_report_hash !== null
    ) {
      throw new Error('EVALUATION_SUPERSESSION_INVALID');
    }
  } else {
