...
function blockContext(manifest: PilotManifestV3, event: PilotEventV3) {
  ...
  if (!assignment) {
    if (
      block.comparative_eligible ||
      event.pilot_arm !== null ||
      block.selected_executor_capability_initial !== 'strong' ||
      block.selected_executor_capability_final_expected !== 'strong'
    )
      throw new Error('ARM_MISMATCH');
    if (
      ...
      throw new Error('BLOCK_IDENTITY_MISMATCH');
...
function expectedAttempt(
  state: PilotBlockStateV3,
): { number: number; kind: ExecutionStarted['payload']['attempt_kind']; next: PilotBlockStateV3 } | null {
...
function validDuration(payload: { started_monotonic_ms: number; finished_monotonic_ms: number; duration_ms: number }): boolean {
  return (
    payload.finished_monotonic_ms >= payload.started_monotonic_ms &&
    payload.duration_ms === payload.finished_monotonic_ms - payload.started_monotonic_ms
  );
}
...
const transitionRows: readonly TransitionRowV3[] = [
  ...
  { from: 'REVIEWING_2', event_type: 'REVIEW_COMPLETED', decision: 'ACCEPT', to: 'READY_ACCEPT' },
  {
    ...
    decision: 'REJECT',
    arms: ['C_ADAPTIVE_EARLY_ESCALATION'],
  ...
  },
  {
    ...
    event_type: 'REVIEW_COMPLETED',
    decision: 'REJECT',
    arms: ['A_STRONG_BASELINE', 'B_CHEAP_NO_EARLY_ESCALATION', 'DIRECT_STRONG'],
    to: 'READY_3',
  },
...
function rowMatches(row: TransitionRowV3, state: PilotBlockStateV3 | null, event: PilotEventV3, arm: Route): boolean {
  const fromMatches = row.from === 'ANY_NON_TERMINAL' ? state !== null && !terminalStates.has(state) : row.from === state;
...
function assertTransitionGuard(
  state: PilotBlockStateV3 | null,
  event: PilotEventV3,
  manifest: PilotManifestV3,
  block: PilotManifestV3['blocks'][number],
  arm: Route,
): void {
  if (event.event_type === 'BLOCK_PLANNED' && event.payload.planned_block_hash !== block.contract_hash)
    throw new Error('PLANNED_BLOCK_HASH_MISMATCH');
  if (
    event.event_type === 'ARM_ASSIGNED' &&
    (event.payload.assigned_arm !== arm || event.payload.assignment_algorithm_version !== manifest.assignment_algorithm_version)
  )
    throw new Error('ASSIGNMENT_MISMATCH');
  if (
    event.event_type === 'ISOLATION_ATTESTED' &&
    (event.payload.base_revision !== block.base_revision ||
      event.payload.clean_tree_hash !== block.clean_tree_hash ||
      event.payload.isolation_policy_version !== manifest.isolation_policy_version ||
      event.payload.isolation_status !== 'CLEAN' ||
      event.payload.observed_tree_hash !== event.payload.clean_tree_hash)
  )
    throw new Error('ISOLATION_INVALID');
  if (event.event_type === 'EXECUTION_STARTED') {
    const expected = state === null ? null : expectedAttempt(state);
    if (!expected || event.payload.attempt_number !== expected.number || event.payload.attempt_kind !== expected.kind)
      throw new Error('ATTEMPT_CEILING_OR_ORDER');
  ...
  if (event.event_type === 'EXECUTION_COMPLETED') {
    ...
    if (
      event.payload.canonical_tree_algorithm_version !== manifest.canonical_tree_algorithm_version ||
      event.payload.volatile_paths_policy_hash !== manifest.volatile_paths_policy_hash ||
      !event.payload.tree_reproduced
    )
      throw new Error('TREE_REPRODUCTION_INVALID');
  }
  if (event.event_type === 'REVIEW_STARTED') {
    ...
    if (
      event.payload.reviewer_binding_ref !== manifest.routing_reviewer_binding_ref ||
      event.payload.reviewer_session_id === event.payload.executor_session_id_reviewed
    )
      throw new Error('REVIEWER_INDEPENDENCE_INVALID');
  }
  if (event.event_type === 'REVIEW_COMPLETED') {
    if (!validDuration(event.payload) || event.payload.review_boundary_hash !== canonicalBoundary(event))
      throw new Error('REVIEW_PROOF_INVALID');
    ...
    if (
      ...
      event.payload.unresolved_finding_ids.length === 0 &&
      event.payload.material_findings.length === 0 &&
      event.payload.non_material_findings.length === 0
    )
      throw new Error('EMPTY_REJECTION');
  }
  if (event.event_type === 'ESCALATION_DECIDED') {
    ...
    if (
      !binding ||
      binding.capability_class !== 'strong' ||
      event.payload.target_capability !== 'strong' ||
      event.payload.decision_policy_version !== manifest.routing_policy_version
    )
      throw new Error('ESCALATION_INVALID');
  }
}
...
function invalid(blockId: string | null, reason: unknown, unresolved: readonly string[] = []): PilotBlockReplay {
  ...
  return {
    state: 'INVALID',
    valid_history: false,
    invalid_reason_codes: [code || 'INVALID_HISTORY'],
    block_id: blockId,
    accepted_revision: null,
    accepted_tree_hash: null,
...
export function replayBlock(manifest: PilotManifestV3, events: readonly PilotEventV3[]): PilotBlockReplay {
  ...
  const history = projected
    .filter((event) => event.block_id === targetBlockId)
  ...
  try {
    for (let index = 0; index < history.length; index += 1) {
      ...
      if (
        ...
        event.event_type === 'ESCALATION_DECIDED' ||
        event.event_type === 'BLOCK_ACCEPTED' ||
        event.event_type === 'BLOCK_FAILED' ||
        event.event_type === 'BLOCK_BLOCKED'
      ) {
      ...
      if (
        event.event_type === 'ISOLATION_ATTESTED' &&
        (event.payload.isolation_status === 'CONTAMINATED' || event.payload.observed_tree_hash !== event.payload.clean_tree_hash)
      )
        throw new Error('WORKSPACE_CONTAMINATED');
      if (event.event_type === 'EXECUTION_STARTED') {
        if (attempts.has(event.payload.attempt_id) || event.payload.attempt_number > 3) throw new Error('ATTEMPT_ID_OR_CEILING_INVALID');
        const expectedInput =
          event.payload.attempt_number === 1 ? block.base_revision : previousReview?.payload.review_boundary_to_revision;
        if (!expectedInput || event.payload.input_revision !== expectedInput) throw new Error('INPUT_REVISION_CHAIN_INVALID');
        if (executorSessions.has(event.payload.executor_session_id) || reviewerSessions.has(event.payload.executor_session_id))
          throw new Error('EXECUTOR_SESSION_REUSED');
        if (
          arm === 'C_ADAPTIVE_EARLY_ESCALATION' &&
          event.payload.attempt_number === 3 &&
          event.payload.executor_binding_ref !== escalationTargetBinding
        )
          throw new Error('ESCALATION_TARGET_BINDING_MISMATCH');
        ...
        if (!currentAttempt || currentAttempt.payload.attempt_id !== event.payload.attempt_id)
          throw new Error('COMPLETION_ATTEMPT_MISMATCH');
        for (const field of [
          'attempt_number',
          'attempt_kind',
          'executor_capability',
          ...
          if (currentAttempt.payload[field] !== event.payload[field]) throw new Error('COMPLETION_OWNERSHIP_MISMATCH');
        if (
          event.payload.canonical_tree_algorithm_version !== manifest.canonical_tree_algorithm_version ||
          event.payload.volatile_paths_policy_hash !== manifest.volatile_paths_policy_hash ||
          !event.payload.tree_reproduced
        ...
        if (event.payload.validation_surface.some((surface) => !block.validation_surface.includes(surface)))
          throw new Error('VALIDATION_SURFACE_INVALID');
        ...
        if (
          !attempt?.completed ||
          attempt.completed !== currentCompletion ||
          event.payload.executor_session_id_reviewed !== attempt.completed.payload.executor_session_id
        )
          throw new Error('REVIEW_ATTEMPT_REFERENCE_INVALID');
        if (
          reviews.has(event.payload.review_id) ||
          executorSessions.has(event.payload.reviewer_session_id) ||
          reviewerSessions.has(event.payload.reviewer_session_id)
        )
          throw new Error('REVIEW_SESSION_OR_ID_INVALID');
        ...
        if (!currentReview || currentReview.payload.review_id !== event.payload.review_id)
          throw new Error('REVIEW_COMPLETION_OWNER_INVALID');
        for (const field of [
          'review_round',
          ...
          if (currentReview.payload[field] !== event.payload[field]) throw new Error('REVIEW_COMPLETION_OWNERSHIP_MISMATCH');
        ...
        if (
          event.payload.review_boundary_from_revision !== expectedFrom ||
          event.payload.review_boundary_to_revision !== attempt.payload.output_revision ||
          event.payload.previous_review_boundary_hash !== expectedPrevious
        )
          throw new Error('REVIEW_BOUNDARY_CHAIN_INVALID');
        const validationHashes = (validations.get(event.payload.reviewed_attempt_id) ?? []).flatMap(
          (validation) => validation.payload.evidence_hashes,
        );
        ...
        if (!sameStrings(event.payload.unresolved_finding_ids, [...nextUnresolved]))
          throw new Error('UNRESOLVED_FINDING_RETENTION_INVALID');
        unresolved.clear();
        ...
        if (event.payload.decision === 'REJECT') rejectedReviewEventId = event.event_id;
          ...
          acceptedReview = event;
          acceptedCompletion = attempt;
        }
        ...
        if (!manifest.binding_registry.some((candidate) => candidate.binding_ref === payload.binding_ref))
          throw new Error('ORCHESTRATOR_OPERATION_BINDING_INVALID');
        ...
        if (
          !binding ||
          payload.pricing_snapshot_id !== manifest.pricing_snapshot.pricing_snapshot_id ||
          payload.currency !== manifest.pricing_snapshot.currency
        )
        ...
        if (payload.orchestrator_operation_id !== null) {
          ...
          if (
            ...
            payload.binding_ref !== operation.payload.binding_ref ||
            payload.attempt_number !== operation.payload.attempt_number
          )
            throw new Error('ORCHESTRATOR_USAGE_OWNERSHIP_MISMATCH');
        } else {
        ...
        if (payload.attempt_id !== null) {
          ...
          if (
            ...
            payload.binding_ref !== attempt.completed.payload.executor_binding_ref ||
            payload.attempt_number !== attempt.completed.payload.attempt_number
          )
            throw new Error('ATTEMPT_USAGE_OWNERSHIP_MISMATCH');
        }
        if (payload.review_id !== null) {
          ...
          if (
            ...
            payload.binding_ref !== review.completed.payload.reviewer_binding_ref ||
            payload.attempt_number !== review.completed.payload.review_round
          )
            throw new Error('REVIEW_USAGE_OWNERSHIP_MISMATCH');
        }
        ...
        if (!review || !ownedReview.window_open || !attempt || review.payload.reviewed_attempt_id !== attempt.payload.attempt_id)
          throw new Error('ORPHAN_OR_MISORDERED_REWORK');
      } else if (event.event_type === 'ESCALATION_DECIDED') {
        if (!rejectedReviewEventId || event.payload.rejected_review_event_id !== rejectedReviewEventId)
          throw new Error('ESCALATION_REVIEW_REFERENCE_INVALID');
        escalationTargetBinding = event.payload.target_binding_ref;
      } else if (event.event_type === 'BLOCK_ACCEPTED') {
        if (
          !acceptedReview ||
          !acceptedCompletion ||
          event.payload.accepted_revision !== acceptedReview.payload.review_boundary_to_revision ||
          event.payload.accepted_revision !== acceptedCompletion.payload.output_revision ||
          event.payload.accepted_tree_hash !== acceptedCompletion.payload.output_tree_hash
        )
          throw new Error('ACCEPTED_REVISION_TREE_INVALID');
      } else if (event.event_type === 'POST_ACCEPT_DEFECT_RECORDED') {
        if (
          !acceptedReview ||
          event.payload.accepted_review_id !== acceptedReview.payload.review_id ||
          event.payload.affected_revision !== acceptedReview.payload.review_boundary_to_revision
        )
          throw new Error('DEFECT_ACCEPTANCE_REFERENCE_INVALID');
      }
