...
export interface RepairControlV4 {
  readonly run_id: string;
  readonly findings: readonly { readonly id: string; readonly evidence_hash: string }[];
}
...
export interface McpAdapterDependenciesV4 {
  readonly client: McpBrokerControlClientV4;
}
...
export function createIpcMcpControlClientV4(client: BrokerIpcClientV4): McpBrokerControlClientV4 {
  return Object.freeze({
    run: (request: RuntimeTaskRequestV4) =>
      client.submit({ type: 'RUN_CODING_TASK', command_id: `mcp-run-${randomBytes(16).toString('hex')}`, request }),
    ...
    getReviewPacket: (runId: string) =>
      client.getReviewPacket?.(runId) ?? Promise.reject(new Error('CAPABILITY_UNVERIFIED: review packet control is unavailable')),
    submitVerdict: (input: McpVerdictControlV4) =>
      client.submitVerdict?.(input) ?? Promise.reject(new Error('CAPABILITY_UNVERIFIED: verdict control is unavailable')),
    close: () => client.close(),
  });
}
...
function replyResult(reply: unknown) {
  let bounded;
  try {
    bounded = mcpReplySchemaV4.parse(reply);
  } catch {
    return publicFailure(new Error('UNKNOWN_FAILURE: invalid broker reply'));
  }
...
function reviewPacketResult(packet: unknown) {
  let bounded;
  try {
    bounded = brokerReviewPacketSchemaV4.parse(packet);
  } catch {
    return publicFailure(new Error('UNKNOWN_FAILURE: invalid review packet'));
  }
  return { content: [{ type: 'text' as const, text: JSON.stringify(bounded) }], structuredContent: bounded };
}
...
export function createBrokerMcpServer(deps: McpAdapterDependenciesV4): McpServerV4 {
  ...
  server.registerTool(
    'run_coding_task',
    {
      description: 'Durably enqueue a complete coding task.',
      inputSchema: V4_MCP_INPUT_SCHEMAS.run_coding_task,
      outputSchema: mcpReplySchemaV4,
    },
    async (request) => {
      try {
        return replyResult(await deps.client.run(request as RuntimeTaskRequestV4));
      } catch (error) {
  ...
  server.registerTool(
    'repair_coding_task',
    {
      description: 'Request one policy-bounded repair using persisted finding identities.',
      inputSchema: V4_MCP_INPUT_SCHEMAS.repair_coding_task,
      outputSchema: mcpReplySchemaV4,
    },
    async (input) => {
      try {
        return replyResult(await deps.client.repair(input));
      } catch (error) {
        return publicFailure(error);
      }
    },
  ...
  server.registerTool(
    ...
    {
      description: 'Request finalization of an already review-accepted run.',
      inputSchema: V4_MCP_INPUT_SCHEMAS.finalize_coding_task,
      outputSchema: mcpReplySchemaV4,
    },
    async ({ run_id }) => {
      try {
        return replyResult(await deps.client.finalize(run_id));
      } catch (error) {
        return publicFailure(error);
      }
    },
  ...
  server.registerTool(
    'abort_coding_task',
    {
      description: 'Durably abort a run without deleting its evidence.',
      inputSchema: V4_MCP_INPUT_SCHEMAS.abort_coding_task,
      outputSchema: mcpReplySchemaV4,
    },
    async ({ run_id }) => {
      try {
        return replyResult(await deps.client.abort(run_id));
      } catch (error) {
        return publicFailure(error);
      }
    },
  ...
  server.registerTool(
    ...
    {
      description: 'Return a bounded status summary for one run.',
      inputSchema: V4_MCP_INPUT_SCHEMAS.get_coding_task_status,
      outputSchema: mcpReplySchemaV4,
    },
    async ({ run_id }) => {
      try {
        return replyResult(await deps.client.status(run_id));
      } catch (error) {
        return publicFailure(error);
      }
    },
  ...
  server.registerTool(
    ...
    {
      description: 'Return the complete, hash-bound review packet after deterministic validation and independent review.',
      inputSchema: V4_MCP_INPUT_SCHEMAS['broker.get_review_packet'],
      outputSchema: brokerReviewPacketSchemaV4,
    },
    async ({ run_id }) => {
      try {
  ...
  server.registerTool(
    ...
    {
      description: 'Persist a hash-bound review verdict. APPROVED does not finalize; REJECTED schedules the existing repair flow.',
      inputSchema: V4_MCP_INPUT_SCHEMAS['broker.submit_verdict'],
      outputSchema: mcpReplySchemaV4,
    },
    async (input) => {
      try {
