...
export function compileHermes(policy: ResolvedPolicy, effectiveWriteIsolation: WriteIsolation = 'degraded'): GeneratedFile[] {
  ...
  const soul = [
    ...
    'Pass each child only a self-contained work contract with id, objective, acceptance_tests, implementation_targets, inputs, constraints, validation commands, success criteria, budget, and result format.',
    'The Economy executor is PROHIBITED from editing the supplied acceptance-test files; it may write only implementation_targets.',
