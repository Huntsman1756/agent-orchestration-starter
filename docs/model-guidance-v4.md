...
The standard NaN/OpenCode snapshot dated `2026-08-16` keeps ChatGPT-authenticated Codex as the read-only orchestrator and independent reviewer. Its Qwen3.6 executor uses direct bounded tool instructions and provider-default inference settings. After one Qwen repair, the economy sequence resolves the separate DeepSeek V4 Flash `escalationExecutor`; a direct frontier route resolves `frontierExecutor` independently. DeepSeek owns both write bindings because NaN documents tool calling for that model, but the two roles require different hash-bound authority. Because those two roles reuse the same provider/model identity, this example demonstrates stronger frontier supervision and separate authority, not a stronger-model execution fallback; `runtime doctor` reports it as `DEGRADED`.

The separate `nan-opencode-glm-premium.example.yaml` snapshot dated `2026-08-21` preserves Qwen3.6 and DeepSeek V4 Flash as economical workers and assigns direct frontier execution to GLM 5.2. NaN's dated API reference lists tool calling and describes the premium model for coding and long-horizon agentic work, so the public-source topology reports `READY`. This is still only a candidate configuration: premium account availability and the exact OpenCode parser, tools, sandbox and task capability require fresh probes and paired evidence. No Codex subscription binding has write authority. Gemma4 is not used as an agentic coding binding because the current NaN API reference does not list tool calling for it. Model identities remain profile data and another qualified provider can replace any binding without changing stable policy.
