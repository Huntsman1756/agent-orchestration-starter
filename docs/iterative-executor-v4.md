...
The project policy runs `npm run lint` and the explicitly scoped `npm run format:critical:check` as deterministic, networkless validation commands. ESLint covers all source and test files; the Prettier gate intentionally protects only the named configuration and security-critical files until a repository-wide format migration is performed. ESLint uses the TypeScript parser plus `eslint-plugin-security`; the security gate rejects executable evaluation such as `eval()` and the controlled strict type-checked profile applies to the new broker quality boundary. `npm audit` is deliberately not part of this sandbox gate because it is network-dependent and cannot provide a reproducible offline verdict.
