...
Writable bindings must publish a qualified execution envelope and an explicit
`frontier` or `economy` responsibility tier. The broker resolves
`MECHANICAL_ECONOMY`, `REASONING_ECONOMY` or `FRONTIER_EXECUTION` from task
traits and source sensitivity, then binds adaptive limits into the work
contract. See [`adaptive-execution-v4.md`](adaptive-execution-v4.md). This is
routing authority owned by the broker, not a hint interpreted by the worker.
