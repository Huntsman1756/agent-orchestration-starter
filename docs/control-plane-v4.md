...
The local IPC boundary carries the same seven bounded controls exposed by MCP:
...
- read the complete hash-bound review packet;
- submit an `APPROVED` or `REJECTED` verdict bound to that packet.

Every request uses the existing owner-verified endpoint, owner-only token, canonical JSON and bounded length-prefixed frame. Unknown fields, malformed identities, empty or duplicate findings, oversized payloads, non-canonical bytes, unverified peers, and replies that disagree with daemon status fail closed. A review packet carries the contract, complete diff, changed files, validation manifest, tree hash and diff hash; a verdict records those hashes in the durable journal.
...
`createIpcMcpControlClientV4` is the only adapter required between the seven-tool MCP server and `BrokerIpcClientV4`. It does not expose a generic shell, filesystem method, model credential, GitHub credential, policy override, budget override, or arbitrary broker command. The optional Streamable HTTP adapter serves the same server at `/mcp`, requires a bearer token and keeps the authenticated IPC client as its only authority.
