...
const diff = {
  changes: [{ path: 'src/greeting.ts', operation: 'MODIFY' as const, content_hash: '1'.repeat(64) }],
  changed_files: 1,
  changed_lines: 2,
...
test('performs exactly one frontier execution, validation, and fresh exact-tree review', async () => {
  ...
  const runner = createFrontierExecutor({
    ...
    fresh_review: async () => ({
      ...
      reviewed_tree_hash: diff.tree_hash,
      reviewed_diff_hash: diff.diff_hash,
    }),
    on_state: (state) => {
      states.push(state);
    },
...
test('validation failure or rejection is terminal and never triggers frontier repair', async () => {
  for (const scenario of ['validation', 'review'] as const) {
    ...
    const runner = createFrontierExecutor({
      ...
      validate: async () => [
        { validation_id: 'test', passed: scenario !== 'validation', result_hash: '7'.repeat(64), validated_tree_hash: diff.tree_hash },
      ],
      fresh_review: async () => ({
        decision: scenario === 'review' ? 'REJECT' : 'ACCEPT',
        reviewer_session_id: 'fresh-review-session',
        reviewed_tree_hash: diff.tree_hash,
        reviewed_diff_hash: diff.diff_hash,
      }),
