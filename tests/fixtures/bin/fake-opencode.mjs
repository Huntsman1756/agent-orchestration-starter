...
const capture = {
  ...
  config,
  hostile_project_config_present: await readFile(join(process.cwd(), 'repo', 'opencode.json'), 'utf8').then(
    () => true,
    () => false,
  ),
};
...
process.stdout.write(
  `${JSON.stringify({ type: 'tool_use', sessionID: 'session_fixture_0001', part: { type: 'tool', tool: 'write', callID: 'call_fixture_0001', state: { status: 'completed', input: { filePath: '/capsule/repo/src/greeting.ts', content: 'done' }, output: 'Wrote file successfully.' } } })}\n`,
);
process.stdout.write(
  `${JSON.stringify({ type: 'step_finish', sessionID: 'session_fixture_0001', part: { type: 'step-finish', reason: 'tool-calls' } })}\n`,
);
process.stdout.write(`${JSON.stringify({ type: 'step_start', sessionID: 'session_fixture_0001', part: { type: 'step-start' } })}\n`);
process.stdout.write(`${JSON.stringify({ type: 'text', sessionID: 'session_fixture_0001', part: { type: 'text', text: 'done' } })}\n`);
process.stdout.write(
  `${JSON.stringify({ type: 'step_finish', sessionID: 'session_fixture_0001', part: { type: 'step-finish', reason: 'stop' } })}\n`,
);
