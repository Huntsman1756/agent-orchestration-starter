...
async function fixtureRepository(): Promise<{ root: string; base: string; emptyConfig: string; emptyHooks: string; sentinels: string[] }> {
  ...
  await git(
    ...
    'config',
    'credential.helper',
    `!node -e "require('fs').writeFileSync('${sentinels[2].replaceAll('\\', '\\\\')}','credential')"`,
  );
...
test('writes and commits exact accepted bytes without invoking hooks, filters, aliases, signing, or the active worktree', async (context) => {
  ...
  const commit = await writer.createCommit({
    tree_sha: tree.tree_sha,
    base_sha: fixture.base,
    message: 'accepted result',
    author_name: 'Runner V4',
    author_email: 'runner@example.invalid',
    authored_at: '2026-08-10T12:00:00Z',
  });
  await writer.updateTaskRef({
    task_ref: 'refs/heads/codex/auto/run-test',
    new_commit_sha: commit.commit_sha,
    expected_old_sha: fixture.base,
  });
  ...
  assert.equal(
    (await exec('git', ['show', `${commit.commit_sha}:src/new.txt`], { cwd: fixture.root, encoding: 'utf8' })).stdout,
    'accepted bytes\n',
  );
...
test('fails closed for stale refs, non-task refs, unsupported modes, and non-sha1 repositories', async (context) => {
  ...
  await assert.rejects(
    writer.updateTaskRef({ task_ref: 'refs/heads/main', new_commit_sha: commit.commit_sha, expected_old_sha: fixture.base }),
    /FINALIZATION_ISOLATION_FAILED/,
  );
  await assert.rejects(
    writer.updateTaskRef({ task_ref: 'refs/heads/codex/auto/stale', new_commit_sha: commit.commit_sha, expected_old_sha: '0'.repeat(40) }),
    /FINALIZATION_FAILED/,
  );
  await assert.rejects(
    writer.writeAcceptedTree({ entries: [{ path: 'device', mode: '120000' as '100644', bytes: Buffer.from('x') }] }),
    /FINALIZATION_ISOLATION_FAILED/,
  );
...
test('uses exact audited plumbing argv and stdin rather than a shell', async () => {
  ...
  const writer = createGitObjectWriter({
    ...
    empty_global_config: 'empty',
    empty_hooks_directory: 'empty-hooks',
  ...
  await writer.updateTaskRef({
    task_ref: 'refs/heads/codex/auto/run',
    new_commit_sha: commit.commit_sha,
    expected_old_sha: '4'.repeat(40),
  });
  assert.deepEqual(
    ...
    [
      ...
      ['mktree', '-z'],
      ['mktree', '-z'],
      ['commit-tree', '3'.repeat(40), '-p', '4'.repeat(40)],
      ['update-ref', 'refs/heads/codex/auto/run', '4'.repeat(40), '4'.repeat(40)],
      ['rev-parse', '--verify', 'refs/heads/codex/auto/run'],
      ['rev-parse', '--verify', `${'4'.repeat(40)}^{tree}`],
    ],
  );
