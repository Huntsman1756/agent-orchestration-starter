...
const validTaskRequest = () => ({ schema_version: 4, task_id: 'TASK-1', request_id: 'req_01HZX3YH8C7Y9QJ4J6M2G5K8N1', repository_id: 'fixture-repo', objective: 'Change the greeting', task_class: 'mechanical-change', requested_risk_class: 'normal', requested_route: 'AUTO', allowed_changes: [{ path: 'src/greeting.ts', operations: ['MODIFY'] }], acceptance_tests: ['tests/greeting.test.ts'], implementation_targets: [{ path: 'src/greeting.ts', operations: ['MODIFY'] }], allowed_validation_ids: ['test'], inputs: [], constraints: [], success_criteria: ['fixture test passes'], max_files_changed: 1, max_changed_lines: 20, max_attempts: 3, prohibited_actions: ['push', 'deploy'], result_schema_version: 4 });
const validRuntimeProfile = () => {
  ...
  const execution = { supportedTaskTraits: ['mechanical', 'localized', 'semantic-debugging', 'cross-file-reasoning', 'multimodal', 'long-horizon', 'architecture', 'security-sensitive', 'migration'], maxSteps: 32, maxToolUses: 64, maxNoMutationSteps: 6, timeoutSeconds: 300, supportsFailedCandidateRepair: false };
  const binding = { harness: 'fixture-harness', provider: 'fixture-provider', model: 'fixture-model', tier: 'frontier', capability: 'fixture-capability', allowedDataScopes: ['SOURCE_CODE_ONLY'], allowedSourceSensitivity: ['PUBLIC'], permissions: 'read-only', guidance, execution };
  return { schemaVersion: 4, id: 'fixture-profile', bindings: { orchestrator: binding, executor: { ...binding, tier: 'economy', permissions: 'contract-write' }, escalationExecutor: { ...binding, tier: 'economy', permissions: 'contract-write' }, frontierExecutor: { ...binding, permissions: 'contract-write' }, reviewer: binding }, runtime: { maxEconomyParallelRequests: 2, maxConcurrentRunsPerRepository: 1 } };
};
...
test('profile and repository policy schemas reject mixed contracts and malformed sensitivity', async () => {
  await assertParity('runtime-profile-v4.schema.json', validRuntimeProfile(), loadRuntimeProfileV4, [
    ...
    (value) => ({ ...value, bindings: { ...value.bindings, executor: { ...value.bindings.executor, tier: 'frontier' } } }),
    (value) => ({ ...value, bindings: { ...value.bindings, executor: { ...value.bindings.executor, execution: undefined } } }),
  ]);
