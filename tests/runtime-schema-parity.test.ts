...
const validRuntimeProfile = () => {
  ...
  return {
    ...
    bindings: {
      ...
      executor: { ...binding, tier: 'economy', permissions: 'contract-write' },
      escalationExecutor: { ...binding, tier: 'economy', permissions: 'contract-write' },
...
const validRepositoryPolicy = () => ({
  ...
  validation: { test: { argv: ['npm', 'test'], workingDirectory: '.', timeoutSeconds: 300, sandboxProfile: 'validation-default' } },
...
async function assertParity(schemaName: string, document: object, loader: Loader, invalidMutations: readonly ((value: any) => any)[]) {
...
test('profile and repository policy schemas reject mixed contracts and malformed sensitivity', async () => {
  await assertParity('runtime-profile-v4.schema.json', validRuntimeProfile(), loadRuntimeProfileV4, [
    ...
    (value) => ({
      ...value,
      bindings: { ...value.bindings, executor: { ...value.bindings.executor, allowedSourceSensitivity: ['PRVIATE'] } },
    }),
    (value) => ({
      ...value,
      bindings: {
        ...value.bindings,
        executor: {
          ...value.bindings.executor,
          guidance: { ...value.bindings.executor.guidance, sourceUrls: ['http://insecure.invalid'] },
        },
      },
    }),
    (value) => ({
      ...value,
      bindings: { ...value.bindings, executor: { ...value.bindings.executor, harness: 'codex', authentication: 'chatgpt-subscription' } },
    }),
...
test('frontier executor result schema and loader reject unknown fields and unsafe paths', async () => {
  await assertParity(
    'frontier-executor-result-v4.schema.json',
    { schema_version: 4, status: 'COMPLETED', summary: 'done', changed_paths: ['src/greeting.ts'] },
    loadFrontierExecutorResultV4,
    [
      (value) => ({ ...value, unexpected: true }),
