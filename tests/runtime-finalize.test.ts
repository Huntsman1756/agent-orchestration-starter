...
function contract(): RuntimeWorkContractV4 {
  const body = {
    ...
    requested_route: 'AUTO' as const,
    allowed_changes: [{ path: 'src/x.ts', operations: ['MODIFY' as const] }],
    acceptance_tests: ['tests/x.test.ts'],
    implementation_targets: [{ path: 'src/x.ts', operations: ['MODIFY' as const] }],
    ...
    result_schema_version: 4 as const,
    run_id: 'run_01HZX3YH8C7Y9QJ4J6M2G5K8N1',
    repository_root_hash: '1'.repeat(64),
    base_sha: baseSha,
    effective_risk_class: 'normal',
    effective_route: 'ECONOMY' as const,
    route_decision_reasons: ['eligible'],
    route_decision_hash: '2'.repeat(64),
    effective_data_scope: 'SOURCE_CODE_ONLY' as const,
    effective_source_sensitivity: 'PUBLIC' as const,
    sandbox_profile_hashes: { executor: '3'.repeat(64) },
...
function diff(): DiffPolicyResultV4 {
  ...
  return {
    ...
    changed_files: 1,
    changed_lines: 1,
    diff_hash: hashCanonicalV4({ schema_version: 4, base_sha: baseSha, changes }),
    tree_hash: hashCanonicalV4({ schema_version: 4, parent_tree: baseSha, applied_changes: changes }),
  };
}

function validation(treeHash: string): ValidationResultV4 {
  const artifact = (kind: 'VALIDATION_STDOUT' | 'VALIDATION_STDERR') => ({
    schema_version: 4 as const,
    kind,
    content_hash: '4'.repeat(64),
    byte_length: 0,
    storage_key: `${kind}/${'4'.repeat(64)}.bin`,
  });
  const body = {
    validation_id: 'test',
    passed: true,
    failure_code: null,
    policy_hash: policyHash,
    sandbox_policy_hash: '5'.repeat(64),
    sandbox_backend_id: 'fixture',
    sandbox_certification_hash: '6'.repeat(64),
    exit_code: 0,
    duration_ms: 1,
    stdout_preview: '',
    stderr_preview: '',
    stdout_artifact: artifact('VALIDATION_STDOUT'),
    stderr_artifact: artifact('VALIDATION_STDERR'),
...
function review(work: RuntimeWorkContractV4, accepted: DiffPolicyResultV4, validationManifestHash: string): ReviewAttestationV4 {
  const body = {
    review_id: 'review-1',
    reviewer_binding_ref: 'reviewer',
    reviewer_session_id: 'fresh-review',
    run_id: work.run_id,
    contract_hash: work.contract_hash,
    base_sha: baseSha,
    reviewed_tree_hash: accepted.tree_hash,
    reviewed_diff_hash: accepted.diff_hash,
    validation_manifest_hash: validationManifestHash,
...
function fixture(): {
  input: FinalizeRunInputV4;
  events: CommitCreatedEventV4[];
  state: { ref: string; releases: string[]; updates: number };
} {
  const work = contract();
  const accepted = diff();
  const result = validation(accepted.tree_hash);
  const manifestHash = hashCanonicalV4({
    schema_version: 4,
    results: [
      {
  ...
  const writer: GitObjectWriterV4 = {
    ...
    updateTaskRef: async (value) => {
      ...
      state.ref = value.new_commit_sha;
      state.updates += 1;
    },
  };
  return {
    ...
    input: {
      ...
      accepted_diff: accepted,
      validation_manifest: { results: [result], manifest_hash: manifestHash },
      review_attestation: review(work, accepted, manifestHash),
      reviewer_session_id: 'fresh-review',
      prior_session_ids: ['executor'],
      task_ref: `refs/heads/codex/auto/${work.run_id}`,
      expected_old_sha: baseSha,
      commit_message: 'accepted',
      author: { name: 'Runner', email: 'runner@example.invalid', timestamp: '2026-08-10T12:00:00Z' },
      writer,
      acquire_run_lock: async () => ({
        release: async () => {
          state.releases.push('run');
        },
      }),
      acquire_repository_lock: async () => ({
        release: async () => {
          state.releases.push('repository');
        },
      }),
      reinspect_diff: async () => structuredClone(accepted),
      snapshot_accepted_tree: async () => [{ path: 'src/x.ts', mode: '100644', bytes }],
      read_task_ref: async () => state.ref,
      append_commit_created: async (event) => {
        events.push(event);
      },
    },
  };
  ...
  assert.deepEqual(events, [
    {
      ...
      review_attestation_hash: input.review_attestation.attestation_hash,
    },
  ]);
});
...
test('rejects changed bytes and every stale contract, diff, validation, review, policy, or profile binding before ref mutation', async () => {
  ...
  const mutations: Array<(input: MutableFinalizeInput) => void> = [
    (input) => {
      input.snapshot_accepted_tree = async () => [{ path: 'src/x.ts', mode: '100644', bytes: Buffer.from('changed') }];
    },
    (input) => {
      input.contract = { ...input.contract, objective: 'forged' };
    },
    (input) => {
      input.expected_policy_hash = '0'.repeat(64);
    },
    (input) => {
      input.expected_profile_hash = '0'.repeat(64);
    },
    (input) => {
      input.reinspect_diff = async () => ({ ...input.accepted_diff, changed_lines: 2 });
    },
    (input) => {
      input.validation_manifest = { ...input.validation_manifest, manifest_hash: '0'.repeat(64) };
    },
    (input) => {
      input.validation_manifest = { ...input.validation_manifest, results: [{ ...input.validation_manifest.results[0]!, passed: false }] };
    },
    (input) => {
      input.review_attestation = { ...input.review_attestation, reviewed_tree_hash: '0'.repeat(64) };
    },
  ];
