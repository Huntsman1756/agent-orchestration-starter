...
import { createDelegationProvenanceV4, verifyDelegationProvenanceV4 } from '../src/runtime/delegation-provenance.js';
...
test('fails closed for missing, forged, stale, or semantically false delegation evidence', () => {
  ...
  assert.throws(
    () => createDelegationProvenanceV4({ ...body, accepted_story_receipt_hashes: [] }, signer),
    /DELEGATION_PROVENANCE_INVALID/u,
  );
});
  ...
  assert.throws(
    () =>
      createDelegationProvenanceV4(
        {
          run_id: 'run_01HZX3YH8C7Y9QJ4J6M2G5K8N1',
          route: 'FRONTIER_EXECUTION',
          disposition: 'FRONTIER_ONLY_EXEMPTION',
          contract_hash: hash('a'),
          policy_hash: hash('b'),
          profile_hash: hash('c'),
          worker_capability_hash: null,
          base_sha: sha('1'),
          commit_sha: sha('2'),
          git_tree_sha: sha('3'),
          evidence_tree_hash: hash('e'),
...
test('CLI verifies the exact evidence binding and fails closed for a different commit', async () => {
  ...
  assert.equal(
    await runCli(
      args.map((value) => (value === sha('2') ? sha('9') : value)),
