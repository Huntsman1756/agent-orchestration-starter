...
import {
  ...
  createMcpStdioAdapter,
  V4_MCP_INSTRUCTIONS,
  type McpBrokerControlClientV4,
} from '../src/mcp/stdio-adapter.js';
...
const reviewPacket = {
  ...
  validation_manifest_hash: 'f'.repeat(64),
  envelope: {},
...
function fakeClient(): McpBrokerControlClientV4 & { calls: string[] } {
  ...
  return {
    calls,
    run: async (value) => {
      calls.push(`run:${value.request_id}`);
      return reply;
    },
    repair: async (value) => {
      calls.push(`repair:${value.run_id}`);
      return reply;
    },
    ...
    abort: async (value) => {
      ...
      return { ...reply, state: 'ABORTED' };
    },
    status: async (value) => {
      calls.push(`status:${value}`);
      return reply;
    },
    getReviewPacket: async (value) => {
      calls.push(`packet:${value}`);
      return reviewPacket;
    },
    submitVerdict: async (value) => {
      calls.push(`verdict:${value.run_id}:${value.verdict}`);
      return reply;
    },
    close: async () => {
      calls.push('close');
...
test('all mutations are short daemon calls and schemas reject unknown authority', async (context) => {
  ...
  await client.callTool({
    name: 'repair_coding_task',
    arguments: { run_id: runId, findings: [{ id: 'finding-1', evidence_hash: 'b'.repeat(64) }] },
  });
  ...
  await client.callTool({
    name: 'broker.submit_verdict',
    arguments: { run_id: runId, packet_hash: reviewPacket.packet_hash, verdict: 'APPROVED', reason: 'Independent review passed.' },
  });
...
test('disconnect and replay preserve daemon idempotency while unavailability is bounded', async () => {
  ...
  broker.run = async (value) => {
    ...
    durableEnqueues += 1;
    accepted.set(value.request_id, reply);
    return reply;
  };
  ...
  assert.deepEqual(
    (await second.client.callTool({ name: 'run_coding_task', arguments: request() as unknown as Record<string, unknown> }))
      .structuredContent,
    reply,
  );
  ...
  unavailable.run = async () => {
    throw new Error('AUTHENTICATION_FAILED: secret local detail');
  };
...
test('bridges all five MCP controls to the authenticated IPC client', async () => {
  ...
  const ipc: BrokerIpcClientV4 = {
    submit: async (command) => {
      calls.push(`run:${command.type === 'RUN_CODING_TASK' ? command.request.request_id : 'invalid'}`);
      return reply;
    },
    repair: async (input) => {
      calls.push(`repair:${input.run_id}:${input.findings[0]!.id}`);
      return reply;
    },
    finalize: async (value) => {
      calls.push(`finalize:${value}`);
