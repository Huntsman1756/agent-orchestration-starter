...
import { assignArms, freezeManifest, verifyManifest, type PilotManifestInputV3 } from '../src/pilot/manifest.js';
    ...
    pricing_snapshot: {
      ...
      tariffs: [
        ...
        },
        {
          ...
          input_token_micro_units_per_token: 3,
          output_token_micro_units_per_token: 4,
          cached_input_token_micro_units_per_token: null,
          reasoning_token_micro_units_per_token: null,
...
function assertManifestRejection(manifest: PilotManifestV3, expectedReason: string, name: string): void {
  ...
  assert.ok(
    result.errors.some((error) => error.includes(expectedReason)),
    `${name} must report ${expectedReason}: ${result.errors.join('; ')}`,
  );
}
...
test('freezing excludes manifest and pricing self-hashes while retaining canonical hashes', () => {
  ...
  const changedSelfHashes = freezeManifest(
    input({
...
test('freezing classifies before deterministic seeded assignment and assigns each comparative triplet exactly once per arm', () => {
  ...
  assert.deepEqual(
    frozen.blocks.map(({ comparative_eligible, cheap_eligible }) => ({ comparative_eligible, cheap_eligible })),
    [
...
test('verification rejects incomplete, unbalanced, duplicate, and non-equivalent comparative triplets', () => {
  ...
  const invalids = [
    [
      'incomplete',
      { ...frozen, blocks: frozen.blocks.slice(0, 2), arm_assignments: frozen.arm_assignments.slice(0, 2) },
      'comparative triplet must have exactly three members',
    ],
    [
      'duplicate assignment',
      { ...frozen, arm_assignments: [...frozen.arm_assignments, frozen.arm_assignments[0]] },
      'duplicate arm assignment',
    ],
    [
      'unbalanced arms',
      {
        ...frozen,
        arm_assignments: frozen.arm_assignments.map((assignment) => ({ ...assignment, pilot_arm: 'A_STRONG_BASELINE' as const })),
      },
      'comparative triplet must contain exact A/B/C membership',
    ],
    [
      'different matching stratum',
      {
        ...frozen,
        blocks: frozen.blocks.map((candidate, index) => (index === 1 ? { ...candidate, matching_stratum: 'localized-low' } : candidate)),
      },
...
test('verification rejects each frozen equivalence mutation independently', () => {
  ...
  for (const [name, mutation] of mutations) {
    const blocks = frozen.blocks.map((candidate, index) =>
      index === 1 ? { ...candidate, ...mutation } : candidate,
    ) as PilotManifestV3['blocks'];
    const expectedReason =
      name === 'matching stratum'
        ? 'comparative triplet must share a matching stratum'
...
test('classification rejects ineligible comparison and keeps direct-to-strong work descriptive and unassigned', () => {
  const invalidMatrices: Array<[Record<string, unknown>, string]> = [
    [
      { cheap_eligible: false, comparative_eligible: true, risk_class: 'low', exclusion_reason: null },
      'comparative_eligible block must be cheap_eligible',
    ],
    [
      { cheap_eligible: true, comparative_eligible: true, risk_class: 'restricted', exclusion_reason: null },
      'comparative_eligible block cannot be restricted',
    ],
    [
      { cheap_eligible: true, comparative_eligible: true, risk_class: 'low', exclusion_reason: 'policy-exclusion' },
      'comparative_eligible block cannot have an exclusion reason',
    ],
  ];
  for (const [invalid, expectedReason] of invalidMatrices) {
    assert.throws(
      () =>
        freezeManifest(
          input({ blocks: [block('block-a', 'triplet-1', invalid), block('block-b', 'triplet-1'), block('block-c', 'triplet-1')] }),
        ),
      (error) => message(error).includes(expectedReason),
    );
  }
  ...
  assert.equal(
    frozen.arm_assignments.some((assignment) => assignment.block_id === direct.block_id),
    false,
  );
  assert.equal(verifyManifest(frozen).comparative_block_count, 3);
  assertManifestRejection(
    { ...frozen, arm_assignments: [...frozen.arm_assignments, { block_id: direct.block_id, pilot_arm: 'A_STRONG_BASELINE' }] },
    'direct block must remain unassigned',
    'direct-to-strong assignment',
  );
});
...
test('comparative blocks remain invalid when an A/B/C assignment is missing', () => {
  ...
  assertManifestRejection(
    { ...frozen, arm_assignments: missing },
    'comparative block lacks arm assignment',
    'missing comparative assignment',
  );
});

test('verification rejects initial and final executor capabilities that contradict each assigned arm', () => {
  const frozen = freezeManifest(input());
  const blockByArm = Object.fromEntries(
    frozen.arm_assignments.map((assignment) => [
  ...
  const invalids: Array<[string, PilotManifestV3['blocks'][number], Record<string, unknown>, string]> = [
    [
      'A initial',
      blockByArm.A_STRONG_BASELINE,
      { selected_executor_capability_initial: 'cheap' },
      'A_STRONG_BASELINE route requires strong initial and final capability',
    ],
    [
      'A final',
      blockByArm.A_STRONG_BASELINE,
    ...
    [
      'B initial',
      blockByArm.B_CHEAP_NO_EARLY_ESCALATION,
      { selected_executor_capability_initial: 'strong' },
      'B_CHEAP_NO_EARLY_ESCALATION route requires cheap initial and final capability',
    ],
    [
      'B final',
      blockByArm.B_CHEAP_NO_EARLY_ESCALATION,
    ...
    [
      'C initial',
      blockByArm.C_ADAPTIVE_EARLY_ESCALATION,
      { selected_executor_capability_initial: 'strong' },
      'C_ADAPTIVE_EARLY_ESCALATION route requires cheap initial and strong final capability',
    ],
    [
      'C final',
      blockByArm.C_ADAPTIVE_EARLY_ESCALATION,
...
test('verification binds registry, common reviewer and policy, isolation policy, pricing snapshot and typed thresholds', () => {
  ...
  const invalids: Array<[string, Partial<PilotManifestV3>, string]> = [
    [
      'duplicate binding registry',
      { binding_registry: [...frozen.binding_registry, { ...frozen.binding_registry[0] }] },
      'binding registry references must be unique',
    ],
    ...
    [
      'pricing snapshot self hash',
      { pricing_snapshot: { ...frozen.pricing_snapshot, pricing_snapshot_hash: hash('9') } },
      'pricing snapshot hash does not match canonical snapshot content',
    ],
    [
      'invalid stage one threshold',
      { stage_thresholds: { ...frozen.stage_thresholds, stage_1_blocks_per_arm: 9 } as unknown as PilotManifestV3['stage_thresholds'] },
      'stage_1_blocks_per_arm',
    ],
    [
      'invalid stage two threshold',
      { stage_thresholds: { ...frozen.stage_thresholds, stage_2_blocks_per_arm: 19 } as unknown as PilotManifestV3['stage_thresholds'] },
      'stage_2_blocks_per_arm',
    ],
    [
      'invalid stage three threshold',
      {
        stage_thresholds: { ...frozen.stage_thresholds, stage_3_max_blocks_per_arm: 29 } as unknown as PilotManifestV3['stage_thresholds'],
      },
      'stage_3_max_blocks_per_arm',
    ],
  ];
  for (const [name, mutation, expectedReason] of invalids)
    assertManifestRejection({ ...frozen, ...mutation } as PilotManifestV3, expectedReason, name);
});
