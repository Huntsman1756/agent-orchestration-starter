...
function event(type: typeof RUNTIME_EVENT_TYPES_V4[number], sequence: number, previous_hash: string | null) {
  return createRuntimeEventV4({ schema_version: 4, type, event_id: `evt_${String(sequence).padStart(16, '0')}`, run_id: runId, sequence, previous_hash, recorded_at: '2026-08-10T12:00:00.000Z', contract_hash: hash, evidence_hashes: [hash] });
}
...
test('accepts every approved bounded event in one strict hash chain and idempotently replays exact IDs', () => {
  ...
  assert.throws(() => appendRuntimeEventV4(log, createRuntimeEventV4({ ...identityDraft, event_id: 'evt_identity-change1', run_id: 'run_01HZX3YH8C7Y9QJ4J6M2G5K8N2' })), /identity/);
});

test('recursively rejects raw context, secret fields, credential-shaped values, and unbounded findings', () => {
  const base = { schema_version: 4 as const, type: 'RUN_FAILED' as const, event_id: 'evt_0000000000000001', run_id: runId, sequence: 1, previous_hash: null, recorded_at: '2026-08-10T12:00:00.000Z', contract_hash: hash, evidence_hashes: [hash] };
  for (const value of [{ ...base, prompt: 'hidden' }, { ...base, nested: { reasoning: 'hidden' } }, { ...base, credential: 'x' }, { ...base, binding_ref: `sk-${'x'.repeat(24)}` }]) assert.throws(() => createRuntimeEventV4(value as never), /telemetry/);
  assert.throws(() => createRuntimeEventV4({ ...base, evidence_hashes: [hash, hash] }), /unique/);
  assert.throws(() => createRuntimeEventV4({ ...base, findings: Array.from({ length: 129 }, (_, index) => ({ id: `f-${index}`, severity: 'low' as const, evidence_hash: hash })) }), /too_big|128|array/i);
});
