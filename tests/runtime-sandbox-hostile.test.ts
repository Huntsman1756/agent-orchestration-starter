...
import { REQUIRED_SANDBOX_EFFECTS_V4, validateSandboxCertificationTranscriptV4 } from '../src/runtime/sandbox-certification.js';
...
const dockerExecutable = dockerConfigured
  ? (process.env.AO_DOCKER_EXECUTABLE ??
...
async function waitForCertificationContainer(): Promise<void> {
  for (let attempt = 0; attempt < 200; attempt += 1) {
    const output = await docker(
      ...
      '--format',
      '{{.Label "agent-orchestration.execution"}}',
    ).catch(() => '');
...
async function writeDockerForwarder(directory: string, blockedExecutionId: string, weakenCertification = false): Promise<void> {
  const source = [
  ...
  await Promise.all(
    ['info', 'image', 'container', 'rm', 'network', 'create', 'start', 'exec', 'inspect', 'ps', 'version'].map(
      async (command) => await writeFile(join(directory, command), source),
    ),
  );
}
...
async function writeNetworkRemovalForwarder(directory: string): Promise<void> {
  ...
  await Promise.all(
    ['info', 'image', 'container', 'rm', 'network', 'create', 'start', 'exec', 'inspect', 'ps', 'version', 'logs'].map(
      async (command) => await writeFile(join(directory, command), source),
    ),
  );
}
...
async function startTlsFixture(options: {
  ...
  await docker(
    ...
    `--ip=${options.address}`,
    `--network-alias=${options.alias}`,
    '--mount',
    `type=bind,src=${options.certificateDirectory},dst=/fixture,readonly`,
    config().image_id,
    'node',
    '-e',
    source,
...
dockerIntegration(
  'production-owned hostile runner records all 17 effects in one bounded live transcript',
  { timeout: 120_000 },
  async () => {
    ...
    assert.deepEqual(transcript.observations.map((observation) => observation.effect).sort(), [...REQUIRED_SANDBOX_EFFECTS_V4].sort());
    assert.deepEqual([...new Set(transcript.artifacts.map((artifact) => artifact.kind))].sort(), [
...
dockerIntegration('Docker effects reject a mount alias introduced before create and preserve its target', { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    assert.equal(
      await docker('ps', '--all', '--quiet', '--no-trunc', '--filter=label=agent-orchestration.execution=exec_hostile_mount_alias_0001'),
      '',
    );
  } finally {
...
dockerIntegration('timeout removes the immutable container ID and preserves a name replacement', { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    await Promise.all(
      [name, preserved, originalId, replacementId]
        .filter((value) => value !== '')
        .map(async (value) => {
...
dockerIntegration(
  'networked executor reaches only the authenticated TLS gateway and never receives the real credential',
  { timeout: 90_000 },
  async () => {
    ...
    try {
      ...
      await docker(
        ...
        '--internal',
        '--label',
        `agent-orchestration.execution=${executionId}`,
        internalNetwork,
      );
      await docker(
        'network',
      ...
      await startTlsFixture({
        name: allowedFixture,
        network: outboundNetwork,
        address: '93.184.216.10',
        alias: 'api.arliai.com',
        certificateDirectory: certAllowed,
      });
      await startTlsFixture({
        name: blockedFixture,
        network: outboundNetwork,
        address: '93.184.216.11',
        alias: 'blocked.example',
        certificateDirectory: certBlocked,
      });
      ...
      const runPromise = certifiedBackend!.run({
        ...
        argv: ['node', '/capsule/network-probe.mjs', lease.gateway_base_url, 'https://blocked.example', 'https://93.184.216.10', '2_000'],
        working_directory: '/capsule',
        environment: {
          HOME: '/tmp/home',
          TMPDIR: '/tmp',
      ...
      const outboundIngress = JSON.parse(
        await docker(
          'exec',
          blockedFixture,
          'node',
      ...
      await Promise.all(
        [internalNetwork, outboundNetwork].map(async (name) => {
...
dockerIntegration(
  'fresh hostile evidence certifies only the exact Docker host, image, policy, and broker',
  { timeout: 60_000 },
  async () => {
    const identity = await inspectDockerSandboxIdentityV4(config(), 'VALIDATION_UNTRUSTED');
    ...
    let replacementId = '';
    try {
      const terminating = assert.rejects(
        () =>
          backend.run({
      ...
      await Promise.all(
        [terminatingName, preservedName, terminatingId, replacementId]
          .filter((value) => value !== '')
          .map(async (value) => {
            await docker('rm', '--force', value).catch(() => undefined);
          }),
...
dockerIntegration('terminate persists cancellation while the production probe is still starting', { timeout: 60_000 }, async () => {
  ...
  assert.equal(await docker('ps', '--all', '--quiet', '--no-trunc', `--filter=label=agent-orchestration.execution=${executionId}`), '');
});
...
dockerIntegration('concurrent production probes share one exact hostile certification run', { timeout: 90_000 }, async () => {
  ...
  const [first, second] = await Promise.all([firstBackend.probe('VALIDATION_UNTRUSTED'), secondBackend.probe('VALIDATION_UNTRUSTED')]);
...
dockerIntegration('independent hostile runners wait out fixed-subnet overlap without false failure', { timeout: 120_000 }, async () => {
  ...
  const launch = async () =>
    await execFileAsync(process.execPath, ['--input-type=module', '-e', source, encodedConfig], {
      cwd: process.cwd(),
      encoding: 'utf8',
      windowsHide: true,
      timeout: 110_000,
...
dockerIntegration('an uncertified forwarding launcher cannot reach Docker create authority', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    replacementId = await docker(
      'create',
      `--name=${original.name}`,
      '--label=agent-orchestration.execution=unrelated',
      '--read-only',
      '--cap-drop=ALL',
      '--security-opt=no-new-privileges',
      '--network=none',
    ...
    await backend.terminate(executionId);
    await running;
    const survivor = (await docker('inspect', originalId).then(
    ...
    await Promise.all(
      [originalId, replacementId, replacementName, `${replacementName}-preserved`].filter(Boolean).map(async (target) => {
...
dockerIntegration(
  'ambiguous network create removes the exact effect, preserves its replacement, and unrelated errors do not retry',
  { timeout: 120_000 },
  async () => {
    const baseConfig = config();
    ...
    for (const mode of ['AMBIGUOUS', 'UNRELATED_ERROR'] as const) {
      ...
      try {
        ...
        const rejected = assert.rejects(
          () => runDockerSandboxHostileCertificationV4({ ...baseConfig, docker_executable: wrapperExecutable }, identity),
          /PROCESS_SANDBOX_UNAVAILABLE/,
        );
        ...
        if (mode === 'AMBIGUOUS') {
          ...
          assert.equal(
            await docker('network', 'ls', '--quiet', '--no-trunc', `--filter=label=agent-orchestration.execution=${created.execution}`),
            '',
          );
        } else {
          await rejected;
...
dockerIntegration(
  'network cleanup propagates permission denial and a later certification retries the exact retained ID',
  { timeout: 180_000 },
  async () => {
...
dockerIntegration(
  'certification binds the immutable Docker launcher and rejects a weakening wrapper or executable replacement',
  { timeout: 120_000 },
  async () => {
    const baseConfig = config();
    assert.equal((await createDockerProcessSandboxV4(baseConfig).probe('VALIDATION_UNTRUSTED')).status, 'SUPPORTED');
      ...
      try {
        ...
        if (mode === 'WEAKENED') {
          assert.equal(
            first.status,
            'UNSUPPORTED',
            'a wrapper that leaks a credential into the hostile process cannot reuse a real Docker certificate',
          );
        } else {
