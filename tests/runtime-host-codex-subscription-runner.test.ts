...
const successfulResult = { exit_code: 0, signal: null, stdout: '', stderr: '', stdout_truncated: false, stderr_truncated: false, termination: null } as const;
  ...
  try {
    const runner = createHostCodexSubscriptionRunnerV4({
      ...
      run_process: async (request) => {
        calls.push(request);
        if (request.argv.at(-2) === 'login' && request.argv.at(-1) === 'status') return { ...successfulResult, stdout: 'Logged in using ChatGPT\n' };
    ...
    const result = await runner.execute({ execution_id: 'exec_host_review_0001', capsule_root: parent, model: 'gpt-5.6-sol', guidance: validModelGuidance(), prompt: 'Review the capsule.', expected_policy_hash: probe.policy_hash });
    assert.equal(result.exit_code, 0);
    const execution = calls.at(-1);
    assert.deepEqual(execution.argv.slice(1, 13), ['exec', '--ephemeral', '--ignore-user-config', '--ignore-rules', '--sandbox', 'read-only', '--skip-git-repo-check', '--output-schema', join(parent, 'review-attestation-v4.schema.json'), '--json', '--cd', parent]);
    ...
    assert.equal(Object.keys(execution.environment).some((key) => /api|token|secret|credential/iu.test(key)), false);
...
test('fails closed when the ChatGPT login is not available from the OS keyring', async () => {
  ...
  try {
    ...
    await assert.rejects(runner.execute({ execution_id: 'exec_host_review_0002', capsule_root: parent, model: 'gpt-5.6-sol', guidance: validModelGuidance(), prompt: 'Review.', expected_policy_hash: probe.policy_hash }), /CAPABILITY_UNVERIFIED/u);
  } finally {
    await rm(parent, { recursive: true, force: true });
  }
});
...
test('rejects a host environment that contains credential-shaped variables', () => {
  assert.throws(() => createHostCodexSubscriptionRunnerV4({ harness_argv: [process.execPath], runtime_home_parent: tmpdir(), environment: { OPENAI_API_KEY: 'forbidden' } }), /CAPABILITY_UNVERIFIED/u);
});
