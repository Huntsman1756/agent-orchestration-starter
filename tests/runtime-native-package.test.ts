...
async function createDisposablePackageProjectForTest(container: string): Promise<string> {
  ...
  await copyFile(join(process.cwd(), 'native', 'linux', 'renameat2-helper.c'), join(project, 'native', 'linux', 'renameat2-helper.c'));
  return project;
}

test(
  'prepack cleans stale native output before a real TypeScript compilation failure',
  {
...
test(
  'clean-only refuses a bind-mounted native root without traversing external contents',
  {
...
test(
  'Linux prepack recovers a legacy cleanup holder within the current invocation',
  {
...
test(
  'Linux prepack fails closed on an inherited current cleanup holder without changing it',
  {
    skip: process.platform !== 'linux' || process.env.AO_NATIVE_PACKAGE_TEST !== '1',
  },
  async () => {
...
test(
  'Linux prepack retains a same-device holder substitution and emits no tarball',
  {
    skip: process.platform !== 'linux' || process.env.AO_NATIVE_PACKAGE_TEST !== '1',
  },
  async () => {
    ...
    try {
      ...
      const retained = await Promise.all(
        (await readdir(cleanupNamespace)).map(async (name) => {
...
test(
  'prepack fails closed on an unsupported build platform',
  {
...
test(
  'Linux prepack cleans a foreign architecture helper and emits the exact native tarball inventory',
  {
    skip: process.env.AO_NATIVE_PACKAGE_TEST !== '1' || process.platform !== 'linux',
  },
  async () => {
