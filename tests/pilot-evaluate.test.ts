...
function manifest(count: number, options: ManifestOptions = {}): PilotManifestV3 {
  ...
  for (let triplet = 1; triplet <= count; triplet += 1) {
    for (let member = 1; member <= 3; member += 1)
      blocks.push({
        block_id: `t-${String(triplet).padStart(2, '0')}-b-${member}`,
        task_id: `task-${triplet}`,
        matching_stratum: 'mechanical-low',
        pair_or_triplet_id: `t-${String(triplet).padStart(2, '0')}`,
        case_fingerprint: hash(String(triplet % 10)),
        contract_hash: hash('c'),
        base_revision: hash('d'),
        clean_tree_hash: hash('e'),
        fixtures_hash: hash('f'),
  ...
  if (options.direct)
    blocks.push({
      block_id: 'direct-strong-1',
      task_id: 'direct-task',
      matching_stratum: 'systemic-high',
      pair_or_triplet_id: 'direct-1',
      case_fingerprint: hash('9'),
      contract_hash: hash('c'),
      base_revision: hash('d'),
      clean_tree_hash: hash('e'),
      fixtures_hash: hash('f'),
      ...
      risk_class: 'high',
      changed_line_band: 'large',
      validation_surface: ['typecheck'],
      cheap_eligible: false,
      comparative_eligible: false,
      routing_selection_reason: 'direct-strong',
      selected_executor_capability_initial: 'strong',
      selected_executor_capability_final_expected: 'strong',
      exclusion_reason: 'not-cheap-eligible',
    });
  ...
  return freezeManifest({
    ...
    routing_reviewer_binding_ref: 'strong-binding',
    ...
    canonical_tree_algorithm_version: 'tree-v1',
    ...
    stage_thresholds: {
      ...
      economic_rejection_rate: options.economicRejection ?? 0.1,
      max_parent_rework_block_rate: options.maxReworkBlockRate ?? 0.1,
      max_parent_rework_production_line_share: 0.1,
      max_escaped_material_defects: options.maxMaterialDefects ?? 0,
      max_escaped_high_defects: options.maxHighDefects ?? 0,
      max_escaped_critical_defects: 0,
      min_observed_cost_completeness: 0.8,
      min_observed_strong_token_completeness: 0.8,
      min_stratum_triplets_for_promotion: options.minSupport ?? 1,
    ...
    pricing_snapshot: {
      ...
      tariffs: [
        {
          binding_ref: 'cheap-binding',
          input_token_micro_units_per_token: 1,
          output_token_micro_units_per_token: 1,
          cached_input_token_micro_units_per_token: 0,
          reasoning_token_micro_units_per_token: 0,
          authoritative_charge_supported: false,
        },
        {
          binding_ref: 'strong-binding',
          input_token_micro_units_per_token: 1,
          output_token_micro_units_per_token: 1,
          cached_input_token_micro_units_per_token: 0,
          reasoning_token_micro_units_per_token: 0,
...
function observation(manifestValue: PilotManifestV3, triplet: number, arm: Arm, options: Options = {}): PilotBlockObservationV3 {
  ...
  const assignment = manifestValue.arm_assignments.find(
    (item) =>
      item.pilot_arm === arm && manifestValue.blocks.find((block) => block.block_id === item.block_id)?.pair_or_triplet_id === tripletId,
  )!;
  const block = manifestValue.blocks.find((item) => item.block_id === assignment.block_id)!;
  const accepted = options.accepted ?? true;
  const valid = options.valid ?? true;
  const cost =
    options.cost === undefined ? (arm === 'A_STRONG_BASELINE' ? 100 : arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? 80 : 90) : options.cost;
  const strong =
    options.strong === undefined
      ? arm === 'A_STRONG_BASELINE'
        ? 1000
        : arm === 'C_ADAPTIVE_EARLY_ESCALATION'
          ? 700
          : 100
      : options.strong;
  const defects = (options.defects ?? []).map((defect, index) => ({
    defect_id: `defect-${triplet}-${arm}-${index}`,
    ...defect,
    discovered_at: timestamp,
    evidence_id: `evidence-${triplet}-${index}`,
    affected_revision: hash('a'),
    category_code: 'correctness',
  }));
  return {
    schema_version: 3,
    pilot_id: manifestValue.pilot_id,
    manifest_hash: manifestValue.manifest_hash,
    task_id: block.task_id,
    block_id: block.block_id,
    matching_stratum: block.matching_stratum,
    pair_or_triplet_id: block.pair_or_triplet_id,
    case_fingerprint: block.case_fingerprint,
    pilot_arm: arm,
    complexity_class: block.complexity_class,
    risk_class: block.risk_class,
    changed_line_band: block.changed_line_band,
    cheap_eligible: true,
    comparative_eligible: true,
    state: valid ? (accepted ? 'ACCEPTED' : 'FAILED') : 'INVALID',
    valid_history: valid,
    invalid_reason_codes: valid ? [] : ['reviewer_session_not_independent'],
    executor_binding_initial: arm === 'A_STRONG_BASELINE' ? 'strong-binding' : 'cheap-binding',
    executor_binding_final: 'strong-binding',
    reviewer_binding_refs: ['strong-binding'],
    execution_attempts: 1,
    repair_rounds: 0,
    escalated: false,
    escalation_reason: null,
    first_pass_accept: accepted,
    accept_after_one_repair: accepted,
    final_accepted: accepted,
    tests_initially_failing: 0,
    tests_finally_passing: accepted ? 1 : 0,
    review_findings_material: 0,
    review_findings_non_material: 0,
    parent_rework_files: { production: options.reworkFile || (options.rework ?? 0) > 0 ? 1 : 0, tests: 0, docs: 0 },
    parent_rework_lines_production: options.rework ?? 0,
    parent_rework_lines_tests: 0,
    parent_rework_lines_docs: 0,
    changed_lines_production: options.changed ?? 10,
    changed_lines_tests: 0,
    ...
    total_usage: {
      operations: 3,
      observed_tokens: strong === null ? null : strong + 200,
      estimated_tokens: strong === null ? null : strong + 220,
    },
    cost_observed: cost,
    cost_estimated: cost,
    cost_observed_completeness: cost === null ? 0 : 1,
    cost_estimated_completeness: cost === null ? 0 : 1,
    strong_tokens_observed: {
      input: aggregate(strong),
      output: aggregate(0),
      cached_input: aggregate(0),
      reasoning: aggregate(0),
      total: aggregate(strong),
    },
    ...
    wall_time_seconds: options.wall === undefined ? 10 : options.wall,
    executor_time_seconds: 5,
    review_time_seconds: 5,
    blocked_cause: null,
    blocked_reason_code: null,
    post_acceptance_window_closed: options.window ?? accepted,
    accepted_at: accepted ? timestamp : null,
    window_opens_at: accepted ? timestamp : null,
    window_closes_at: accepted ? '2026-08-08T12:01:00.000Z' : null,
    post_accept_defects: defects,
    post_accept_defects_count: defects.length,
    post_accept_max_severity: defects.length
      ? defects
          .map((item) => item.severity)
          .sort()
          .at(-1)!
      : null,
    late_quality_evidence_count: options.late ?? 0,
    quality_warnings: (options.late ?? 0) > 0 ? ['LATE_QUALITY_EVIDENCE'] : [],
...
function observations(manifestValue: PilotManifestV3, configure: (triplet: number, arm: Arm) => Options = () => ({})) {
  ...
  return Array.from({ length: count }, (_, index) => index + 1).flatMap((triplet) =>
    arms.map((arm) => observation(manifestValue, triplet, arm, configure(triplet, arm))),
  );
}
...
function gate(manifestValue: PilotManifestV3, stage: 1 | 2 | 3, options: { eligible?: boolean; seed?: string } = {}): PilotRoutingGateV3 {
  ...
  return {
    ...
    strata_policy: [...new Map(manifestValue.blocks.map((block) => [block.matching_stratum, block])).values()]
      ...
      .map((block) => {
        const structurallyEligible =
          block.complexity_class !== 'systemic' && block.risk_class !== 'high' && block.risk_class !== 'restricted';
    ...
    thresholds: {
      ...
      material_improvement_rate: source.material_improvement_rate,
      economic_rejection_rate: source.economic_rejection_rate,
...
function evaluatePilot(
  ...
  if (second.decision === 'REJECT' || second.decision === 'PROMOTE_BOUNDED') {
    ...
    const fallback = observations(manifestValue, (triplet, arm) =>
      arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: triplet <= 10 ? 70 : 100, strong: triplet <= 10 ? 700 : 1000 } : {},
  ...
  assert.throws(
    () =>
      evaluatePilot(
        frozen,
        values,
        { ...gate(frozen, 2), thresholds: { ...gate(frozen, 2).thresholds, material_improvement_rate: 0.16 } } as PilotRoutingGateV3,
        context(),
      ),
    /GATE_THRESHOLD_MISMATCH/,
  );
  assert.throws(
    () => evaluatePilot(frozen, values, { ...gate(frozen, 2), strata_policy: [] } as PilotRoutingGateV3, context()),
    /STRATA_POLICY_INVALID/,
  );
  assert.throws(
    () =>
      evaluatePilot(
        frozen,
        values,
        { ...gate(frozen, 2), strata_policy: [...gate(frozen, 2).strata_policy, ...gate(frozen, 2).strata_policy] } as PilotRoutingGateV3,
        context(),
      ),
    /STRATA_POLICY_INVALID/,
  );
...
test('evaluates the frozen Stage 1 prefix without refilling an excluded candidate', () => {
  ...
  const values = observations(frozen).filter(
    (value) => !(value.pair_or_triplet_id === 't-01' && value.pilot_arm === 'B_CHEAP_NO_EARLY_ESCALATION'),
  );
...
test('Stage 1 never materializes a promoted stratum record', () => {
  ...
  assert.deepEqual(
    report.not_validated_strata,
    report.strata.map((stratum) => stratum.matching_stratum),
  );
});
...
test('hard rejection never materializes a promoted stratum record', () => {
  ...
  const values = observations(frozen, (triplet, arm) =>
    triplet === 11 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { defects: [{ severity: 'high', material: true }] } : {},
  );
...
test('is invariant across observation order and freezes resampling metadata and gate policy hash', () => {
  ...
  assert.deepEqual(forward.interval_metadata, {
    confidence_level: 0.8,
    interval_algorithm_version: 'paired-bootstrap-sha256-counter-v1',
    resampling_iterations: 100,
    resampling_seed: 'bootstrap-seed-v1',
  });
  assert.equal(forward.gate_policy_hash, reverse.gate_policy_hash);
  assert.notEqual(
    forward.gate_policy_hash,
    evaluatePilot(frozen, values, gate(frozen, 2, { seed: 'other-seed' }), context()).gate_policy_hash,
  );
});

test('Stage 1 continues when usable branches fail the point gate', () => {
  const frozen = manifest(10);
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: 95, strong: 950 } : {})),
    gate(frozen, 1),
    context(),
  );
  assert.equal(report.decision, 'CONTINUE');
  assert.deepEqual(
    report.efficiency_branches.map((branch) => branch.status),
...
test('Stage 1 rejects an observable material defect hard condition', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) =>
      triplet === 1 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { defects: [{ severity: 'high', material: true }] } : {},
    ),
    gate(frozen, 1),
    context(),
  );
...
test('complete observed cost with zero A and positive C is a hard economic regression', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => ({
...
test('concrete reducer invalid reasons propagate through exclusion and report evidence', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) => (triplet === 1 && arm === 'A_STRONG_BASELINE' ? { valid: false } : {})),
    gate(frozen, 1),
    context(),
  );
...
test('integrity hard reject outranks Stage 2 sample insufficiency', () => {
  const frozen = manifest(20);
  const values = observations(frozen).filter(
    (value) => !(value.pair_or_triplet_id === 't-20' && value.pilot_arm === 'B_CHEAP_NO_EARLY_ESCALATION'),
  );
...
test('known comparative integrity mismatch outside the frozen prefix does not reject Stage 1', () => {
  const frozen = manifest(20);
  const values = observations(frozen).map((value) =>
    value.pair_or_triplet_id === 't-20' && value.pilot_arm === 'A_STRONG_BASELINE'
...
test('Stage 2 promotes only passing local strata and emits schema-valid bounded evidence', () => {
  ...
  assert.deepEqual(
    report.strata.map((item) => [item.matching_stratum, item.status]),
    [['mechanical-low', 'PROMOTED']],
  );
});
...
test('Stage 2 rejects sufficiently complete observed cost regression', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: triplet <= 10 ? 80 : 140 } : {})),
    gate(frozen, 2),
    context(),
  );
  assert.equal(report.decision, 'REJECT');
...
test('incomplete cost emits observed_cost_incomplete in both branch and insufficient outcome', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: null, strong: null } : {})),
    gate(frozen, 2),
    context(),
  );
...
test('Stage 2 continues when only the global paired quality lower bound is negative', () => {
  ...
  const values = observations(frozen, (triplet, arm) => {
    ...
    if (triplet === 2 && arm === 'C_ADAPTIVE_EARLY_ESCALATION')
      return { defects: [{ severity: 'low', material: true }], cost: 70, strong: 700 };
    return arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: 70, strong: 700 } : {};
  });
  const report = evaluatePilot(frozen, values, gate(frozen, 2), context());
...
test('observed cost regression is hard only when its frozen completeness threshold is met', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) =>
      arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: triplet <= 5 ? null : 110, strong: null } : {},
    ),
    gate(frozen, 2),
    context(),
  );
...
test('hard defect ceilings use the frozen gate threshold rather than a zero default', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) =>
      triplet === 11 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { defects: [{ severity: 'high', material: false }] } : {},
    ),
    gate(frozen, 2),
    context(),
  );
...
test('a critical post-accept defect independently hard rejects', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) =>
      triplet === 1 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { defects: [{ severity: 'critical', material: false }] } : {},
    ),
    gate(frozen, 1),
    context(),
  );
...
test('a high post-accept defect independently hard rejects', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) =>
      triplet === 1 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { defects: [{ severity: 'high', material: false }] } : {},
    ),
    gate(frozen, 1),
    context(),
  );
...
test('hard rework ceilings use the frozen gate threshold', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' && triplet >= 11 && triplet <= 13 ? { rework: 1 } : {})),
    gate(frozen, 2),
    context(),
  );
...
test('parent rework block rate independently hard rejects with the normative reason', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' && triplet <= 2 ? { reworkFile: true } : {})),
    gate(frozen, 1),
    context(),
  );
  assert.equal(report.decision, 'REJECT');
...
test('parent rework production line share independently hard rejects with the normative reason', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (triplet, arm) => (triplet === 1 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { rework: 20, changed: 100 } : {})),
    gate(frozen, 1),
    context(),
  );
...
test('hard economic regression uses the frozen rejection threshold', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: 115 } : {})),
    gate(frozen, 2),
    context(),
  );
  assert.equal(report.efficiency_branches[1].status, 'PASS');
...
test('Stage 2 cannot promote when candidate wall time is incomplete', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { wall: null } : {})),
    gate(frozen, 2),
    context(),
  );
  assert.equal(report.decision, 'INSUFFICIENT_EVIDENCE');
...
test('complete candidate wall time above baseline independently hard rejects', () => {
  ...
  const report = evaluatePilot(
    frozen,
    observations(frozen, (_triplet, arm) => (arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { wall: 11 } : {})),
    gate(frozen, 1),
    context(),
  );
  assert.equal(report.decision, 'REJECT');
...
test('Stage 1 report hashes quality and operational evidence from the full input multiset', () => {
  ...
  const changedLater = observations(frozen, (triplet, arm) =>
    triplet === 20 && arm === 'A_STRONG_BASELINE' ? { cost: 200, defects: [{ severity: 'low', material: false }], late: 1 } : {},
  );
  ...
  assert.notEqual(
    first.operational_totals.comparative_by_arm.A_STRONG_BASELINE.cost_observed.known_sum,
...
test('bootstrap resource aggregation fails closed before losing a safe integer unit', () => {
  const frozen = manifest(20);
  const values = observations(frozen, (triplet, arm) =>
    triplet === 1 && arm === 'A_STRONG_BASELINE' ? { cost: Number.MAX_SAFE_INTEGER - 1900 } : {},
  );
...
test('Stage 3 independently terminates ambiguous evidence as inconclusive', () => {
  ...
  assert.equal(
    evaluatePilot(
      frozen,
      observations(frozen, (triplet, arm) =>
        arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { cost: triplet <= 15 ? 70 : 100, strong: triplet <= 15 ? 700 : 1000 } : {},
      ),
      gate(frozen, 3),
      context(),
    ).decision,
...
test('Stage 3 below 30 admitted triplets is terminal inconclusive', () => {
  const frozen = manifest(30);
  const values = observations(frozen).filter(
    (value) => !(value.pair_or_triplet_id === 't-30' && value.pilot_arm === 'B_CHEAP_NO_EARLY_ESCALATION'),
  );
...
test('a rejected pilot cannot advance to a new primary look', () => {
  ...
  assert.throws(
    () => evaluatePilot(frozen, observations(frozen), gate(frozen, 2), context(2, rejected)),
    /TERMINAL_DECISION_CANNOT_ADVANCE/,
  );
});
...
test('same-stage Stage 3 quality reevaluation may preserve an existing bounded promotion', () => {
  ...
  const qualityOnly = observations(frozen, (triplet, arm) =>
    triplet === 1 && arm === 'A_STRONG_BASELINE' ? { defects: [{ severity: 'low', material: false }], late: 1 } : {},
  );
...
test('non-retry append leaves mutable caller reports untouched and returns a detached frozen snapshot', () => {
  const frozen = manifest(10);
  const first = evaluatePilot(frozen, observations(frozen), gate(frozen, 1), context());
  const withQuality = observations(frozen, (triplet, arm) => (triplet === 1 && arm === 'A_STRONG_BASELINE' ? { late: 1 } : {}));
...
test('quality reevaluation requires monotonic owner-bound evidence and exact prior supersession', () => {
  ...
  const withQuality = observations(frozen, (triplet, arm) =>
    triplet === 1 && arm === 'A_STRONG_BASELINE' ? { defects: [{ severity: 'low', material: false }], late: 1 } : {},
  );
  const second = evaluatePilot(frozen, withQuality, gate(frozen, 1), context(2, first));
  assert.deepEqual(
    [second.supersedes_evaluation_id, second.supersedes_evaluation_version, second.expected_superseded_report_hash],
    [first.evaluation_id, first.evaluation_version, hashCanonical(first)],
  );
  assert.ok(second.warnings.includes('STALE_DECISION'));
  assert.throws(
    () => appendEvaluation(appendEvaluation([], first), { ...second, quality_evidence_count: 0 }),
    /QUALITY_EVIDENCE_DECREASED/,
  );
});

test('same-stage reevaluation admits a newly closed quality window with no defects', () => {
  const frozen = manifest(10);
  const openWindow = observations(frozen, (triplet, arm) =>
    triplet === 1 && arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? { window: false } : {},
  );
...
test('same-stage decision input hash includes direct observations', () => {
  ...
  const changed = initial.map((value) =>
    value.pilot_arm === null
      ? ({ ...value, cost_observed: 101 } as PilotBlockObservationV3)
      : value.block_id === initial[0].block_id
        ? ({ ...value, late_quality_evidence_count: 1 } as PilotBlockObservationV3)
        : value,
  );
...
test('same-stage decision input hash includes newly supplied unknown observations', () => {
  ...
  const first = evaluatePilot(frozen, initial, gate(frozen, 1), context());
  const withLate = initial.map((value, index) =>
    index === 0 ? ({ ...value, late_quality_evidence_count: 1 } as PilotBlockObservationV3) : value,
  );
