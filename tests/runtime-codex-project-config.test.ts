...
function policy(): ResolvedPolicy {
  const role = {
    ...
    modelRef: 'provider/frontier-model',
  ...
  return {
    ...
    roles: {
      ...
      executor: {
        ...
        tier: 'economy',
        model: 'economy-model',
        modelRef: 'provider/economy-model',
        permissions: { read: true, write: true },
      },
      reviewer: role,
    },
    validation: { commands: ['npm test'] },
    routing: { strategies: ['orchestrated'] },
...
test('renders a central immutable runtime binding without embedding credentials', () => {
  const generated = renderCodexProjectConfig({
    frontier_model: 'frontier-model',
    reasoning_effort: 'high',
    runtime_entrypoint: 'G:/host/runtime.mjs',
    activation_manifest: 'G:/repo/.agent-orchestration/activation-v4.json',
  });
  assert.match(
    generated.content,
    /args = \["G:\/host\/runtime\.mjs", "runtime", "mcp-stdio", "--activation", "G:\/repo\/\.agent-orchestration\/activation-v4\.json"\]/u,
  );
