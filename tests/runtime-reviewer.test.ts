...
test('uses a fresh read-only capsule session and never mounts the worktree', async () => {
  ...
  const result = await reviewer.review({ execution_id: 'exec_review_0001', binding: { role: 'reviewer', binding: { harness: 'codex', provider: 'frontier', model: 'frontier-model', capability: 'review', allowedDataScopes: ['SOURCE_CODE_ONLY'], allowedSourceSensitivity: ['PUBLIC'], permissions: 'read-only', guidance: validModelGuidance() }, binding_hash: '9'.repeat(64) }, capability, envelope: envelope(), capsule_parent: 'C:/parent', forbidden_roots: ['C:/worktree'], expected_sandbox_policy_hash: 'a'.repeat(64), prior_session_ids: ['executor-session'] });
