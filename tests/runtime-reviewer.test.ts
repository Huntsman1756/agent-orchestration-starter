...
const capability = await probeRuntimeBinding({
  identity,
  probed_at: '2026-08-10T09:00:00.000Z',
  ttl_seconds: 7200,
...
test('uses a fresh read-only capsule session and never mounts the worktree', async () => {
  ...
  const reviewer = createReviewer({
    ...
    credentials: {
      lease: async () => ({
        ...
        internal_network: 'ao-int-exec-review-0001',
  ...
  const result = await reviewer.review({
    ...
    capability,
    envelope: envelope(),
    capsule_parent: 'C:/parent',
    forbidden_roots: ['C:/worktree'],
  ...
  assert.deepEqual(requests[0].argv.slice(1, 13), [
    'exec',
    '--ephemeral',
    '--ignore-user-config',
    '--ignore-rules',
...
test('reviews through the host ChatGPT subscription runner without leasing gateway credentials', async () => {
  ...
  const reviewer = createReviewer({
    subscription_runner: {
      probe: async () => {
        throw new Error('execute owns the fresh qualification probe');
      },
  ...
  const result = await reviewer.review({
    execution_id: 'exec_review_subscription',
    binding: {
      role: 'reviewer',
      binding: {
        harness: 'codex',
...
test('encodes exactly economy, one repair, typed model escalation, and final review', async () => {
  ...
  const machine = createEconomyReviewSequence({
    execute_economy: async (input) => {
      calls.push(`${input.role}-${input.attempt}-${input.repair_finding_hashes.length}`);
      return attemptResult(`${input.role}-${input.attempt}`);
    },
    execute_escalation: async (input) => {
      calls.push(`${input.role}-${input.failure_evidence_hashes.length}-${input.escalation_decision_hash.length}`);
      return attemptResult(input.role);
    },
    validate: async (_attempt, ordinal) => {
      calls.push(`validate-${ordinal}`);
      return true;
    },
    review: async (_attempt, ordinal) => {
      calls.push(`review-${ordinal}`);
      return reviews[ordinal - 1]!;
    },
  });
...
test('repairs deterministic validation evidence before spending reviewer work', async () => {
  ...
  const machine = createEconomyReviewSequence({
    execute_economy: async (input) => {
      calls.push(`execute-${input.attempt}-${input.repair_finding_hashes.length}`);
      return attemptResult(`attempt-${input.attempt}`);
    },
    execute_escalation: async () => {
      throw new Error('unexpected escalation');
    },
    validate: async (_attempt, ordinal) => {
      calls.push(`validate-${ordinal}`);
      return ordinal === 1 ? { passed: false, finding_hashes: [finding] } : { passed: true, finding_hashes: [] };
    },
    review: async (_attempt, ordinal) => {
      calls.push(`review-${ordinal}`);
      return acceptance('validation-repair');
    },
  });
...
test('permits one content-addressed context expansion in a second fresh session', async () => {
  ...
  const reviewer = createReviewer({
    ...
    capability_identity_for: () => identity,
    now: () => '2026-08-10T10:00:00.000Z',
    build_capsule: async (input) => {
      capsuleContextSizes.push(input.approved_context.length);
      return { root: `C:/review-${capsuleContextSizes.length}`, manifest_hash: 'f'.repeat(64) };
    },
    resolve_context: async () => ({ path: 'src/requested.ts', content, content_hash: contentHash }),
    persist_attestation: async () => {},
  });
  const result = await reviewer.review({
    execution_id: 'exec_review_context',
    binding: {
      role: 'reviewer',
      binding: {
        harness: 'codex',
...
function attemptResult(session_id: string) {
  return {
    session_id,
    events: [],
    diff: { changes: [], changed_files: 0, changed_lines: 0, diff_hash: 'd'.repeat(64), tree_hash: 'c'.repeat(64) },
    capability_snapshot_hash: '8'.repeat(64),
  };
}
function rejection(seed: string) {
  const value = acceptance(seed);
  return {
    ...value,
    decision: 'REJECT' as const,
    findings: [{ id: `finding-${seed}`, severity: 'high', message: 'fix' }],
    unresolved_finding_ids: [`finding-${seed}`],
  };
}
function acceptance(seed: string) {
  return { ...attestation(), review_id: seed, attestation_hash: hashCanonicalV4({ seed }) } as any;
}
function reviewResult(session: string, decision: 'REQUEST_CONTEXT' | 'ACCEPT', requested: string[]) {
  const body = {
    ...attestation(),
    reviewer_session_id: session,
    decision,
    requested_context_hashes: requested,
    attestation_hash: undefined,
  };
  delete (body as any).attestation_hash;
...
function envelope() {
  const body = {
    schema_version: 4,
    contract: { run_id: 'run_01HZX3YH8C7Y9QJ4J6M2G5K8N1', contract_hash: 'a'.repeat(64) },
  ...
  return { ...body, envelope_hash: hashCanonicalV4(body) } as any;
}
function attestation() {
  const body = {
    review_id: 'review-01',
    reviewer_binding_ref: '9'.repeat(64),
