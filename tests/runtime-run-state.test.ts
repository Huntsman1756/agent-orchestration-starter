...
test('durably reaches review acceptance only from exact post-reinspection evidence', () => {
  ...
  const executing = reduceBrokerStateV4(ready, {
    ...
    command_id: 'started',
    run_id: acceptedCommand.run_id,
    process: { pid: 42, boot_nonce: 'process-boot' },
  });
  const waiting = reduceBrokerStateV4(executing, {
    type: 'ATTEMPT_RECORDED',
    command_id: 'attempt',
  ...
  const reinspected = reduceBrokerStateV4(waiting, {
    type: 'PATHS_REINSPECTED',
    command_id: 'reinspect',
    run_id: acceptedCommand.run_id,
    inspection_epoch: 2,
  });
  const evidence = {
...
test('records a hash-bound frontier verdict without turning approval into publication', () => {
  ...
  const executing = reduceBrokerStateV4(ready, {
    type: 'EXTERNAL_PROCESS_STARTED',
    command_id: 'started-for-verdict',
    run_id: acceptedCommand.run_id,
    process: { pid: 43, boot_nonce: 'process-boot' },
  });
  const waiting = reduceBrokerStateV4(executing, {
    type: 'ATTEMPT_RECORDED',
    command_id: 'attempt-for-verdict',
    run_id: acceptedCommand.run_id,
    attempt: { attempt: 1, executor_binding_ref: 'fixture-executor', result_hash: 'b'.repeat(64) },
  });
  const reinspected = reduceBrokerStateV4(waiting, {
    type: 'PATHS_REINSPECTED',
    command_id: 'reinspect-for-verdict',
    run_id: acceptedCommand.run_id,
    inspection_epoch: 2,
  });
  ...
  const verdictBody = {
    schema_version: 4,
    run_id: acceptedCommand.run_id,
    review_packet_hash: '1'.repeat(64),
    contract_hash: acceptedCommand.contract.contract_hash,
    diff_hash: reviewed.runs[acceptedCommand.run_id].result.diff_hash,
    tree_hash: reviewed.runs[acceptedCommand.run_id].result.tree_hash,
  ...
  assert.throws(
    () =>
      reduceBrokerStateV4(approved, {
        ...verdict,
        command_id: 'verdict-replayed',
        verdict: 'REJECTED',
        verdict_hash: hashCanonicalV4({ ...verdictBody, verdict: 'REJECTED' }),
      }),
    /BROKER_STATE_CORRUPT/,
  );
  assert.throws(
    () => reduceBrokerStateV4(reviewed, { ...verdict, command_id: 'verdict-forged', diff_hash: '4'.repeat(64) }),
    /BROKER_STATE_CORRUPT/,
  );
});
...
test('records authenticated abort as a distinct terminal transition', () => {
  const acceptedCommand = accepted();
  const ready = reduceBrokerStateV4(initialBrokerStateV4(), acceptedCommand);
  const aborted = reduceBrokerStateV4(ready, {
    type: 'RUN_ABORTED',
    command_id: 'abort',
    run_id: acceptedCommand.run_id,
    failure: { code: 'ABORTED', message: 'ABORTED: authenticated control', retryable: false, evidence_hashes: [] },
  });
  ...
  assert.throws(
    () =>
      reduceBrokerStateV4(ready, {
        type: 'RUN_ABORTED',
        command_id: 'abort',
        run_id: acceptedCommand.run_id,
        failure: { code: 'UNKNOWN_FAILURE', message: 'wrong', retryable: false, evidence_hashes: [] },
      }),
    /BROKER_STATE_CORRUPT/,
  );
  assert.throws(
    () =>
      reduceBrokerStateV4(aborted, {
        type: 'RUN_ABORTED',
        command_id: 'abort-again',
        run_id: acceptedCommand.run_id,
        failure: { code: 'ABORTED', message: 'ABORTED: again', retryable: false, evidence_hashes: [] },
      }),
    /BROKER_STATE_CORRUPT/,
  );
});
...
test('rejects every lifecycle transition that bypasses execution, reinspection, or terminal gates', () => {
  const acceptedCommand = accepted();
  const ready = reduceBrokerStateV4(initialBrokerStateV4(), acceptedCommand);
  const startedCommand: BrokerCommandV4 = {
    type: 'EXTERNAL_PROCESS_STARTED',
    command_id: 'started',
    run_id: acceptedCommand.run_id,
    process: { pid: 42, boot_nonce: 'process-boot' },
  };
  const attemptCommand: BrokerCommandV4 = {
    type: 'ATTEMPT_RECORDED',
    command_id: 'attempt',
    run_id: acceptedCommand.run_id,
    attempt: { attempt: 1, executor_binding_ref: 'fixture-executor', result_hash: 'b'.repeat(64) },
  };
  const reinspectCommand: BrokerCommandV4 = {
    type: 'PATHS_REINSPECTED',
    command_id: 'reinspect',
    run_id: acceptedCommand.run_id,
    inspection_epoch: 2,
  ...
  const failed = reduceBrokerStateV4(executing, {
    type: 'RUN_FAILED',
    command_id: 'failed',
    run_id: acceptedCommand.run_id,
    failure: { code: 'UNKNOWN_FAILURE', message: 'indeterminate', retryable: false, evidence_hashes: [] },
  });
  for (const command of [
    ...
    {
      type: 'RUN_FAILED',
      command_id: 'failed-again',
      run_id: acceptedCommand.run_id,
      failure: { code: 'UNKNOWN_FAILURE', message: 'again', retryable: false, evidence_hashes: [] },
    } as const,
  ]) {
...
test('publication is durable from the accepted commit through the verified merge', () => {
  const acceptedCommand = accepted();
  const reviewedCommand: Extract<BrokerCommandV4, { type: 'RUN_ACCEPTED' }> = {
    ...acceptedCommand,
    result: {
      ...validRuntimeResult(),
  ...
  const command: Extract<BrokerCommandV4, { type: 'COMMIT_CREATED' }> = {
    type: 'COMMIT_CREATED',
    command_id: 'commit-created',
    run_id: acceptedCommand.run_id,
    task_ref: `refs/heads/codex/auto/${reviewedCommand.run_id}`,
    base_sha: reviewedCommand.contract.base_sha,
    git_tree_sha: 'c'.repeat(40),
    evidence_tree_hash: reviewedCommand.result.tree_hash,
    commit_sha: 'd'.repeat(40),
    contract_hash: reviewedCommand.contract.contract_hash,
    diff_hash: reviewedCommand.result.diff_hash,
  ...
  const pushed = reduceBrokerStateV4(committed, {
    type: 'BRANCH_PUSHED',
    command_id: 'branch-pushed',
    run_id: command.run_id,
    commit_sha: command.commit_sha,
    branch: reviewedCommand.result.branch,
    remote: 'origin',
    publication_policy_hash: reviewedCommand.contract.policy_hash,
  });
  assert.equal(pushed.runs[command.run_id].result.state, 'PUBLICATION_PUSHED');
  const prUrl = 'https://github.com/acme/repo/pull/17';
  const opened = reduceBrokerStateV4(pushed, {
    type: 'PULL_REQUEST_RECORDED',
    command_id: 'pr-recorded',
    run_id: command.run_id,
    commit_sha: command.commit_sha,
    pull_request: 17,
  ...
  const checked = reduceBrokerStateV4(opened, {
    type: 'REQUIRED_CHECKS_PASSED',
    command_id: 'checks-passed',
    run_id: command.run_id,
    commit_sha: command.commit_sha,
    pull_request: 17,
    publication_policy_hash: reviewedCommand.contract.policy_hash,
  });
  const merged = reduceBrokerStateV4(checked, {
    type: 'RUN_MERGED',
    command_id: 'run-merged',
    run_id: command.run_id,
    commit_sha: command.commit_sha,
    pull_request: 17,
    pull_request_url: prUrl,
  ...
  assert.throws(
    () =>
      reduceBrokerStateV4(pushed, {
        type: 'RUN_MERGED',
        command_id: 'early-merge',
        run_id: command.run_id,
        commit_sha: command.commit_sha,
        pull_request: 17,
        pull_request_url: prUrl,
...
test('rejects an atomic cache snapshot that disagrees with journal replay', async () => {
  ...
  await journal.close();
  await writeFile(
    join(directory, 'current-state.v4.json'),
    JSON.stringify({ sequence: 1, state_hash: '0'.repeat(64), state: initialBrokerStateV4() }),
  );
...
test('propagates a directory fsync failure after atomic cache rename', async () => {
  ...
  await assert.rejects(
    () =>
      writeWithDurability(directory, initialBrokerStateV4(), 0, {
        syncDirectory: async () => {
          throw fsyncError;
        },
      }),
