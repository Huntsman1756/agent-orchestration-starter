...
import {
  ...
  evaluateRuntimeBindingHealthV4,
  runtimeBindingHealthAllowsV4,
} from '../src/runtime/binding-health.js';
...
const policy = {
  ...
  cooldownSeconds: 60,
  recoveryCanarySuccesses: 2,
} as const;
function observation(
  id: string,
  recordedAt: string,
  outcome: Parameters<typeof createRuntimeBindingHealthObservationV4>[0]['outcome'],
  taskTrait: 'mechanical' | 'semantic-debugging' = 'mechanical',
) {
  return createRuntimeBindingHealthObservationV4({
    schemaVersion: 4,
    observationId: `health_${id.padEnd(8, '0')}`,
    bindingHash,
    taskTrait,
    recordedAt,
    outcome,
...
test('quarantines only the failing exact binding and task trait', () => {
  ...
  const otherTrait = evaluateRuntimeBindingHealthV4({
    bindingHash,
    taskTrait: 'semantic-debugging',
    observations: [],
    policy,
    evaluatedAt: '2026-08-21T10:01:01.000Z',
  });
...
test('requires cooldown and clean canaries before restoring normal routing', () => {
  ...
  const cooled = evaluateRuntimeBindingHealthV4({
    bindingHash,
    taskTrait: 'mechanical',
    observations,
    policy,
    evaluatedAt: '2026-08-21T10:02:00.000Z',
  });
  ...
  const restored = evaluateRuntimeBindingHealthV4({
    ...
    observations: [
      ...observations,
      observation('three', '2026-08-21T10:02:01.000Z', 'CANARY_ACCEPTED'),
      observation('four', '2026-08-21T10:03:01.000Z', 'CANARY_ACCEPTED'),
    ],
    policy,
...
test('critical false acceptance quarantines immediately and evidence is immutable', () => {
  ...
  const snapshot = evaluateRuntimeBindingHealthV4({
    ...
    observations: [value],
    policy,
    evaluatedAt: '2026-08-21T10:00:01.000Z',
  });
  assert.equal(snapshot.status, 'QUARANTINED');
  assert.throws(
    () =>
      evaluateRuntimeBindingHealthV4({
        bindingHash,
        taskTrait: 'mechanical',
        observations: [{ ...value, outcome: 'ACCEPTED' }],
        policy,
        evaluatedAt: '2026-08-21T10:00:01.000Z',
      }),
    /observation hash/,
...
test('rejects ambiguous snapshots for the same binding and task trait', () => {
  ...
  assert.throws(
    () =>
      runtimeBindingHealthAllowsV4({
        snapshots: [snapshot, { ...snapshot, evaluatedAt: '2026-08-21T10:00:02.000Z', snapshotHash: 'b'.repeat(64) }],
        bindingHash,
        taskTraits: ['mechanical'],
      }),
    /ambiguous/,
  );
});
