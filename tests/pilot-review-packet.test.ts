...
function input(): ReviewPacketInputV3 {
  return {
    ...
    block_id: 'block-a',
    review_id: 'review-2',
    reviewed_attempt_id: 'attempt-2',
    contract_hash: hash('a'),
    case_fingerprint: hash('b'),
    review_boundary_from_revision: hash('c'),
    review_boundary_to_revision: hash('d'),
    review_input_diff_hash: hash('e'),
    previous_review_boundary_hash: hash('f'),
    unresolved_finding_ids: ['finding-1'],
    validation_evidence_hashes: [hash('1')],
...
test('buildReviewPacket rejects malformed hashes, unbounded collections, duplicate findings, and unsupported context requests', () => {
  const cases: Array<[string, ReviewPacketInputV3]> = [
    ...
    [
      'unsupported request',
      { ...input(), bounded_context_requests: [{ request_type: 'RAW_DIFF' as 'VALIDATION_EVIDENCE', evidence_hash: hash('2') }] },
    ],
  ];
