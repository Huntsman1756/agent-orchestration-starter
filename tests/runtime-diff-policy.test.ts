...
test('classifies exact create, modify, and delete paths and emits reproducible hashes', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      first.changes.map(({ path, operation }) => ({ path, operation })),
      [
...
test('red-team economy diff cannot modify an immutable acceptance test and carries a repair instruction', async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: unknown) =>
        error instanceof EconomyPolicyViolationErrorV4 &&
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        return (
          packet.findings[0]?.instruction === error.repair_instruction && packet.findings[0]?.category_code === 'economy_policy_violation'
        );
      },
    );
  } finally {
