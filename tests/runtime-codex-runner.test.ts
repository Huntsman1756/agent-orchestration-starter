...
function binding(): ResolvedBindingV4 {
  return {
    ...
    binding: {
      ...
      guidance: { ...validModelGuidance(), reasoningEffort: 'high' },
    },
...
test('runs exact ephemeral Codex argv from the capsule with a frozen bounded prompt', async () => {
  ...
  const runner = createCodexRunner({
    sandbox: localSandbox(requests),
    credentials,
    harness_argv: [process.execPath, fakeCodex],
  ...
  const result = await runner.execute({
    execution_id: 'exec_codex_0001',
    binding: binding(),
    capability,
    capsule_root: capsule,
    worktree_root: worktree,
    instruction_manifest_hash: '4'.repeat(64),
    contract: contract(),
  ...
  assert.deepEqual(request.argv.slice(2, 14), [
    'exec',
    '--ephemeral',
    '--ignore-user-config',
    '--ignore-rules',
...
test('rejects stale isolation before credential lease or harness launch', async () => {
  ...
  let leased = 0;
  const unavailable: ProcessSandboxBackendV4 = {
    id: 'none',
    probe: async () => ({ status: 'UNSUPPORTED', failure: 'PROCESS_SANDBOX_UNAVAILABLE' }),
    run: async () => {
      throw new Error('launched');
    },
    terminate: async () => {},
  };
  const guarded = createCodexRunner({
    sandbox: unavailable,
    credentials: {
      ...credentials,
      lease: async (value) => {
        leased += 1;
        return credentials.lease(value);
      },
    },
    harness_argv: ['codex'],
    now: () => '2026-08-10T08:01:00.000Z',
  ...
  await assert.rejects(
    () =>
      guarded.execute({
        execution_id: 'exec_codex_bad1',
        binding: binding(),
        capability,
        capsule_root: 'C:/capsule',
...
function contract(): RuntimeWorkContractV4 {
  ...
  return {
    ...
    requested_risk_class: 'high',
    effective_risk_class: 'high',
    requested_route: 'FRONTIER' as const,
    effective_route: 'FRONTIER' as const,
    route_decision_reasons: ['frontier required'],
    ...
    implementation_targets: [{ path: 'src/greeting.ts', operations: ['MODIFY' as const] }],
    ...
    max_attempts: 1,
    sandbox_profile_hashes: { frontier: hash },
