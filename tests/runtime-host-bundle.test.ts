...
test('the installed host bundle starts without a project node_modules tree', async () => {
  ...
  const result = spawnSync(process.execPath, [installation.entrypoint], {
    cwd: hostRoot,
    encoding: 'utf8',
...
test('the installed bundle loads only its hash-pinned host driver through a repository activation', async () => {
  ...
  await writeFile(
    policyPath,
    (await readFile(new URL('../policies/repository-policy.example.yaml', import.meta.url), 'utf8')).replace(
      'example-repository',
      'driver-repository',
    ),
  );
  ...
  const result = spawnSync(
    process.execPath,
    [installation.entrypoint, 'runtime', 'mcp-stdio', '--activation', join(repositoryRoot, '.agent-orchestration', 'activation-v4.json')],
    { cwd: repositoryRoot, encoding: 'utf8', env: { PATH: process.env.PATH ?? '' }, windowsHide: true },
  );
