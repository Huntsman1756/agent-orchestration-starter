...
import {
  publishFinalizedRunV4,
  skipFinalizedRunPublicationV4,
  type PublicationAdapterV4,
  type PublicationProgressEventV4,
  type PullRequestV4,
} from '../src/runtime/publication.js';
...
function fixture(existing: PullRequestV4 | null = null) {
  ...
  const finalized: FinalizedRunV4 = {
    run_id: contract.run_id,
    task_ref: `refs/heads/codex/auto/${contract.run_id}`,
    commit_sha: commitSha,
    git_tree_sha: '7'.repeat(40),
    evidence_tree_hash: contract.route_decision_hash,
    diff_hash: '6'.repeat(64),
    review_attestation_hash: '5'.repeat(64),
  };
  const branch = `codex/auto/${contract.run_id}`;
  const open: PullRequestV4 = {
    number: 17,
  ...
  const adapter: PublicationAdapterV4 = {
    pushExact: async (input) => {
      calls.push(`push:${input.commit_sha}:${input.branch}:${input.remote}`);
      return { remote_sha: input.commit_sha };
    },
  ...
  return {
    ...
    input: {
      ...
      title: 'Automated accepted change',
      body: 'Validated and independently reviewed.',
      adapter,
      ...
      append_publication_event: async (event: PublicationProgressEventV4) => {
        events.push(event);
      },
    },
  };
...
test('pushes the accepted commit, creates an exact PR, waits for required checks, merges, and journals', async () => {
  ...
  assert.deepEqual(
    ...
    ['BRANCH_PUSHED', 'PULL_REQUEST_RECORDED', 'REQUIRED_CHECKS_PASSED', 'RUN_MERGED'],
  );
  assert.deepEqual(value.events.at(-1), {
    type: 'RUN_MERGED',
    command_id: `run-merged:${value.contract.run_id}`,
    run_id: value.contract.run_id,
    commit_sha: commitSha,
    pull_request: 17,
    pull_request_url: value.merged.url,
    merge_commit_sha: mergeSha,
    publication_policy_hash: value.policyHash,
  });
});
...
test('recovers idempotently from an already merged exact PR', async () => {
  ...
  assert.deepEqual(
    value.events.map((event) => event.type),
    ['BRANCH_PUSHED', 'PULL_REQUEST_RECORDED', 'RUN_MERGED'],
  );
});
...
test('fails closed on disabled or stale policy, prohibited publication, and stale PR identity', async () => {
  ...
  await assert.rejects(
    publishFinalizedRunV4({
      ...disabled.input,
      policy: { ...disabled.policy, publication: { ...disabled.policy.publication, enabled: false } },
    }),
  ...
  await assert.rejects(
    publishFinalizedRunV4({
      ...prohibited.input,
      contract: { ...body, contract_hash: hashCanonicalV4(body) } as unknown as RuntimeWorkContractV4,
    }),
    /PUBLICATION_POLICY_DENIED/,
  );
...
test('releases both locks when publication fails', async () => {
  const value = fixture();
  value.input.adapter.pushExact = async () => {
    throw new Error('PUBLICATION_FAILED: offline');
  };
...
test('an opt-in publication gate accepts only signed evidence for the exact finalized run', async () => {
  ...
  const evidence = createDelegationProvenanceV4(
    {
      ...
      profile_hash: value.contract.profile_hash,
      worker_capability_hash: '1'.repeat(64),
      base_sha: value.contract.base_sha,
      commit_sha: value.finalized.commit_sha,
      git_tree_sha: value.finalized.git_tree_sha,
      evidence_tree_hash: value.finalized.evidence_tree_hash,
      diff_hash: value.finalized.diff_hash,
      validation_manifest_hash: '4'.repeat(64),
      review_attestation_hash: value.finalized.review_attestation_hash,
      accepted_story_receipt_hashes: ['6'.repeat(64)],
      frontier_decision_hashes: [],
      exemption: null,
  ...
  await publishFinalizedRunV4({
    ...
    delegation_provenance_gate: {
      ...
      evidence,
      trusted_public_key: keys.publicKey.export({ type: 'spki', format: 'pem' }).toString(),
    },
  });
  ...
  await assert.rejects(
    publishFinalizedRunV4({
      ...
      delegation_provenance_gate: {
        enforcement: 'REQUIRED',
        evidence: { ...evidence, diff_hash: '0'.repeat(64) },
        trusted_public_key: keys.publicKey.export({ type: 'spki', format: 'pem' }).toString(),
      },
    }),
    /DELEGATION_PROVENANCE_REQUIRED/u,
...
test('records an explicit terminal skip only when policy or contract prohibits publication', async () => {
  const disabled = fixture();
  const policy = loadRuntimeRepositoryPolicyV4({
    ...validRepositoryPolicy(),
    publication: { ...validRepositoryPolicy().publication, enabled: false },
  });
  const policyHash = hashCanonicalV4(policy);
  const body = { ...disabled.contract, policy_hash: policyHash } as Record<string, unknown>;
  delete body.contract_hash;
  const contract = { ...body, contract_hash: hashCanonicalV4(body) } as unknown as RuntimeWorkContractV4;
  await skipFinalizedRunPublicationV4({
    ...disabled.input,
    contract,
    finalized: { ...disabled.finalized, run_id: contract.run_id, task_ref: `refs/heads/codex/auto/${contract.run_id}` },
    policy,
    expected_policy_hash: policyHash,
  });
