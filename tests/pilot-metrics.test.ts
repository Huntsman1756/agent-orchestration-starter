...
function manifest(tripletCount: number, includeDirect = false): PilotManifestV3 {
  ...
  for (let triplet = 1; triplet <= tripletCount; triplet += 1) {
    for (let member = 1; member <= 3; member += 1) {
      blocks.push({
        ...
        complexity_class: triplet % 2 === 0 ? 'localized' : 'mechanical',
        risk_class: triplet % 2 === 0 ? 'medium' : 'low',
  ...
  return freezeManifest({
    ...
    stage_thresholds: {
      ...
      min_stratum_triplets_for_promotion: 1,
...
function observationForBlock(
  ...
  const defects = (options.defects ?? []).map((defect, index) => ({
    defect_id: `defect-${block.block_id}-${index}`,
    ...defect,
  ...
  const estimatedCost = complete(
    options.cost_estimated,
    arm === 'A_STRONG_BASELINE' ? 110 : arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? 88 : 99,
  );
  const strongObserved = complete(
    options.strong_observed,
    arm === 'A_STRONG_BASELINE' ? 1000 : arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? 700 : 100,
  );
  const strongEstimated = complete(
    options.strong_estimated,
    arm === 'A_STRONG_BASELINE' ? 1100 : arm === 'C_ADAPTIVE_EARLY_ESCALATION' ? 770 : 110,
  );
  ...
  return {
    ...
    state: finalOutcome,
    valid_history: validHistory,
    invalid_reason_codes: validHistory ? [] : ['invalid_event_history'],
    ...
    execution_attempts: options.escalated ? 3 : 1,
    repair_rounds: 0,
    escalated: options.escalated ?? false,
    escalation_reason: options.escalated ? 'review-rejected' : null,
    first_pass_accept: accepted && (options.first_pass ?? true),
    ...
    parent_rework_lines_production: options.parent_rework_lines ?? 0,
    parent_rework_lines_tests: 0,
    parent_rework_lines_docs: 0,
    changed_lines_production: options.changed_production_lines ?? 10,
    changed_lines_tests: 0,
    changed_lines_docs: 0,
    orchestrator_usage: { operations: 1, observed_tokens: 100, estimated_tokens: 110 },
    executor_usage: {
      operations: 1,
      observed_tokens: allRoleObserved === null ? null : Math.max(0, allRoleObserved - 200),
      estimated_tokens: allRoleEstimated === null ? null : Math.max(0, allRoleEstimated - 220),
    },
    ...
    cost_observed: observedCost,
    cost_estimated: estimatedCost,
    cost_observed_completeness: observedCost === null ? 0 : 1,
    cost_estimated_completeness: estimatedCost === null ? 0 : 1,
    strong_tokens_observed: {
      input: aggregate(strongObserved),
      output: aggregate(0),
      cached_input: aggregate(0),
      reasoning: aggregate(0),
      total: aggregate(strongObserved),
    },
    strong_tokens_estimated: {
      input: aggregate(strongEstimated),
      output: aggregate(0),
      cached_input: aggregate(0),
      reasoning: aggregate(0),
      total: aggregate(strongEstimated),
    },
...
test('admits only exact complete triplets and retains missing-member and direct operational evidence', () => {
  ...
  assert.deepEqual(
    {
      candidate: result.denominators.candidate_triplets,
      admitted: result.denominators.admitted_triplets,
      excluded: result.denominators.excluded_triplets,
    },
    { candidate: 2, admitted: 1, excluded: 1 },
  );
...
const exclusionCases: Array<[string, (triplet: PilotBlockObservationV3[]) => PilotBlockObservationV3[]]> = [
      ...
      triplet.map((value, index) =>
        index === 0
          ? ({
              ...
              state: 'INVALID',
              invalid_reason_codes: ['invalid_event_history'],
  ...
  [
    ...
    (triplet) => triplet.map((value, index) => (index === 2 ? { ...value, case_fingerprint: hash('8') } : value)),
  ],
  [
    'quality_window_open',
    (triplet) => triplet.map((value, index) => (index === 2 ? { ...value, post_acceptance_window_closed: false } : value)),
  ],
];
...
test('attributes each mismatched observation once by its expected manifest block identity', () => {
  ...
  assert.equal(
    result.exclusions[0].operational_resources.cost_observed.known_sum,
...
test('reports literal paired acceptance and quality discordance counts over the same admitted triplets', () => {
  ...
  assert.deepEqual(result.metrics.by_arm.A_STRONG_BASELINE.final_acceptance_rate, {
    numerator: 2,
    denominator: 4,
    value: 0.5,
    confidence_interval: null,
  });
  assert.deepEqual(result.metrics.by_arm.C_ADAPTIVE_EARLY_ESCALATION.final_acceptance_rate, {
    numerator: 2,
    denominator: 4,
    value: 0.5,
    confidence_interval: null,
...
test('uses ratio-of-sums rework and symmetric economic populations without mixing observed and estimated tokens', () => {
  ...
  const observations = [
    ...completeTriplet(frozen, 1, {
      C_ADAPTIVE_EARLY_ESCALATION: {
        parent_rework_lines: 2,
        changed_production_lines: 4,
        cost_observed: 80,
        all_role_observed: 1000,
        all_role_estimated: 1200,
      },
    }),
    ...completeTriplet(frozen, 2, {
      ...
      C_ADAPTIVE_EARLY_ESCALATION: {
        accepted: false,
        parent_rework_lines: 0,
        changed_production_lines: 6,
        cost_observed: 20,
        all_role_observed: 500,
        all_role_estimated: 700,
      },
    }),
  ];
  const result = computePilotMetrics(frozen, observations);
  ...
  assert.deepEqual(candidate.all_role_tokens_observed_per_accepted_block, {
    numerator: 1500,
    denominator: 1,
    value: 1500,
    confidence_interval: null,
  });
  assert.deepEqual(candidate.all_role_tokens_estimated_per_accepted_block, {
    numerator: 1900,
    denominator: 1,
    value: 1900,
    confidence_interval: null,
...
test('hashes the complete observation multiset canonically while preserving duplicate and direct evidence', () => {
  ...
  const direct = observationForBlock(
    frozen,
    frozen.blocks.find((block) => block.block_id === 'direct-strong-1')!,
    null,
  );
...
test('counts every escaped material, high and critical defect retained in accepted work', () => {
  ...
  const observations = [
    ...completeTriplet(frozen, 1, {
      C_ADAPTIVE_EARLY_ESCALATION: {
        defects: [
          { severity: 'high', material: true },
          { severity: 'critical', material: true },
        ],
      },
    }),
    ...completeTriplet(frozen, 2, { C_ADAPTIVE_EARLY_ESCALATION: { accepted: false } }),
...
test('hashes owned closures, in-window defects, and late-count markers into the combined quality evidence', () => {
  ...
  const increasedLate = triplet.map((value) =>
    value.pilot_arm === 'A_STRONG_BASELINE' ? { ...value, late_quality_evidence_count: 2 } : value,
  ) as PilotBlockObservationV3[];
...
test('keeps zero-operation economics descriptively zero but outside every observed branch population', () => {
  const frozen = manifest(1);
  const zeroOperation = completeTriplet(frozen, 1).map(
    (value) =>
      ({
        ...value,
...
test('zero-operation nonzero economics canonicalize to zero and trigger integrity failure', () => {
  const frozen = manifest(1);
  const invalid = completeTriplet(frozen, 1).map((value, index) =>
    index === 0
      ? ({
          ...value,
