...
This is not yet a production-ready one-command runner. The current branch deliberately stops before exposing saved ChatGPT/Codex authentication or an API key to repository-controlled processes. The authenticated IPC protocol covers run, status, repair, finalize and abort, but native host handlers, certified verifier/coordinator composition, and installation of the immutable project-local runtime bundle remain incomplete. Until those are implemented and certified on the target host, `runtime mcp-stdio` exits with `CAPABILITY_UNVERIFIED` instead of falling back to direct edits. See [`docs/control-plane-v4.md`](docs/control-plane-v4.md).
